...
- **Git Workflow**: Trunk Based Development (TBD). Every plan resolves to one of four **Delivery Modes**: `worktree-to-pr` (worktree → draft PR → `[HUMAN]` merge -- **the repo-wide default**), `worktree-to-origin-main` (worktree → direct push, `[AI]`), `main-to-origin-main` (primary checkout → direct push, `[AI]`), `main-to-pr` (primary checkout → draft PR → `[HUMAN]` merge). `*-to-pr` modes run the **PR-Review Maker→Fixer Cycle** (`pr-review-maker` / `pr-review-fixer`, default 3 sequential CI-gated cycles) before the human merge -- meeting the review cycle's done-definition and flipping the PR to ready (which is when the [PR Merge Protocol](./repo-governance/development/workflow/pr-merge-protocol.md) approval gate fires) is not the same as the PR being merged; the merge itself stays on the human's own schedule. See the [Trunk Based Development Convention](./repo-governance/development/workflow/trunk-based-development.md#default-delivery-mode-worktree-to-pr), the [Git Push Default Convention](./repo-governance/development/workflow/git-push-default.md), and the [Plans Organization Convention §Delivery Mode](./repo-governance/conventions/structure/plans.md#delivery-mode) for the full three-tier precedence (invocation argument > plan `## Delivery Mode` field > default) and mechanics.
...
11. **PR Review Cycle**: `pr-review-maker`, `pr-review-fixer` — GitHub-Reviews-API-driven maker→fixer

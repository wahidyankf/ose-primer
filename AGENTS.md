...
5. **PR Review Cycle** — `pr-review-scout-maker` classifies each cycle's risk tier and assembles the
   shared context brief, the discipline `pr-review-*-maker` specialists it selects fan out to
   `pr-review-synthesis-maker` (coordinator), which hands off to `pr-review-fixer`, for `*-to-pr`
   Delivery Mode plans.

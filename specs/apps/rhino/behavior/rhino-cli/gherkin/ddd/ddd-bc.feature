...
Feature: Bounded-Context Structural Parity Validation
  ...
  As a developer
  I want rhino-cli ddd bc to verify the DDD registry matches the filesystem
  ...
  Scenario: Severity warn flag downgrades findings to warnings and exits zero
    ...
    When the developer runs "rhino-cli ddd bc organiclever --severity=warn"
    Then the command exits successfully
  ...
  Scenario: Registry file not found for unknown app is an error
    When the developer runs "rhino-cli ddd bc unknownapp"
    Then the command exits with a failure code

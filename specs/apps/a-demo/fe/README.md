...
| --------------------- | ----------------------- | ------------------------ | ---- |
| `a-demo-fe-ts-nextjs` | Next.js 16 (App Router) | @amiceli/vitest-cucumber | 3301 |

E2E tests are centralized in `a-demo-fe-e2e` (Playwright + playwright-bdd v8), which tests any
frontend via `BASE_URL` env var against `a-demo-be-java-springboot` on port 8201.
...
- Frontend runs against `a-demo-be-java-springboot` with real PostgreSQL
...
2. Create the folder if it does not exist: `specs/apps/a-demo/fe/gherkin/[domain]/`

...
- Shared Playwright suite in `apps/a-demo-be-e2e/`
...
| --------------------------- | ------------------- | ----------------------- | ----------------------- | ---------- |
| a-demo-be-java-springboot   | Java (Spring)       | Cucumber JVM + mocks    | Cucumber JVM + PG       | Playwright |
| a-demo-be-elixir-phoenix    | Elixir (Phoenix)    | Cabbage + mocks         | Cabbage + Ecto/PG       | Playwright |
| a-demo-be-fsharp-giraffe    | F# (Giraffe)        | TickSpec + mocks        | TickSpec + Npgsql/PG    | Playwright |
| a-demo-be-golang-gin        | Go (Gin)            | Godog + mocks           | Godog + pgx/PG          | Playwright |
| a-demo-be-python-fastapi    | Python (FastAPI)    | pytest-bdd + mocks      | pytest-bdd + asyncpg/PG | Playwright |
| a-demo-be-rust-axum         | Rust (Axum)         | cucumber-rs + mocks     | cucumber-rs + sqlx/PG   | Playwright |
| a-demo-be-kotlin-ktor       | Kotlin (Ktor)       | Cucumber JVM + mocks    | Cucumber JVM + PG       | Playwright |
| a-demo-be-java-vertx        | Java (Vert.x)       | Cucumber JVM + mocks    | Cucumber JVM + PG       | Playwright |
| a-demo-be-ts-effect         | TypeScript (Effect) | Cucumber.js + mocks     | Cucumber.js + pg/PG     | Playwright |
| a-demo-be-csharp-aspnetcore | C# (ASP.NET Core)   | Reqnroll + mocks        | Reqnroll + Npgsql/PG    | Playwright |
| a-demo-be-clojure-pedestal  | Clojure (Pedestal)  | kaocha-cucumber + mocks | kaocha-cucumber + PG    | Playwright |
...
2. Create the folder if it does not exist: `specs/apps/a-demo/be/gherkin/[context]/`

...
Feature: Internal Link Validation for oseplatform-fs

  As a content author
  I want to validate internal links in oseplatform-fs content
  ...
  Scenario: A content directory with all valid internal links passes validation
    Given oseplatform-fs content where all internal links resolve correctly
    When the developer runs links check
    Then the command exits successfully

  Scenario: A broken internal link is detected and reported
    Given oseplatform-fs content with a link pointing to a non-existent page
    When the developer runs links check
    Then the command exits with a failure code

  Scenario: External URLs are not validated
    Given oseplatform-fs content with only external HTTPS links
    When the developer runs links check
    Then the command exits successfully

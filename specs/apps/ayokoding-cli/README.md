...
`cmd/**/*.go` or `specs/apps/ayokoding-cli/**/*.feature` change. The `test:unit`
...
3. Create `apps/ayokoding-cli/cmd/<command>.integration_test.go` (integration test with godog):
...
4. Update the `inputs` list in `test:unit` and `test:integration` in `apps/ayokoding-cli/project.json` if needed
...
Coverage is measured at the unit level only (≥90% line coverage).

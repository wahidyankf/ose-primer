...
Platform-agnostic Gherkin acceptance specifications for a crud-scale backend service covering
...
- Shared Playwright suite in `apps/crud-be-e2e/`
...
| ------------------------- | ------------------- | ----------------------- | ----------------------- | ---------- |
| crud-be-java-springboot   | Java (Spring)       | Cucumber JVM + mocks    | Cucumber JVM + PG       | Playwright |
| crud-be-elixir-phoenix    | Elixir (Phoenix)    | Cabbage + mocks         | Cabbage + Ecto/PG       | Playwright |
| crud-be-fsharp-giraffe    | F# (Giraffe)        | TickSpec + mocks        | TickSpec + Npgsql/PG    | Playwright |
| crud-be-golang-gin        | Go (Gin)            | Godog + mocks           | Godog + pgx/PG          | Playwright |
| crud-be-python-fastapi    | Python (FastAPI)    | pytest-bdd + mocks      | pytest-bdd + asyncpg/PG | Playwright |
| crud-be-rust-axum         | Rust (Axum)         | cucumber-rs + mocks     | cucumber-rs + sqlx/PG   | Playwright |
| crud-be-kotlin-ktor       | Kotlin (Ktor)       | Cucumber JVM + mocks    | Cucumber JVM + PG       | Playwright |
| crud-be-java-vertx        | Java (Vert.x)       | Cucumber JVM + mocks    | Cucumber JVM + PG       | Playwright |
| crud-be-ts-effect         | TypeScript (Effect) | Cucumber.js + mocks     | Cucumber.js + pg/PG     | Playwright |
| crud-be-csharp-aspnetcore | C# (ASP.NET Core)   | Reqnroll + mocks        | Reqnroll + Npgsql/PG    | Playwright |
| crud-be-clojure-pedestal  | Clojure (Pedestal)  | kaocha-cucumber + mocks | kaocha-cucumber + PG    | Playwright |
...
enhancement to support crud-be test file naming patterns (e.g., `health_steps_test.go`,

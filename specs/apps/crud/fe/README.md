...
| ------------------- | ----------------------- | ------------------------ | ---- |
| `crud-fe-ts-nextjs` | Next.js 16 (App Router) | @amiceli/vitest-cucumber | 3301 |

E2E tests are centralized in `crud-fe-e2e` (Playwright + playwright-bdd v8), which tests any
frontend via `BASE_URL` env var against `crud-be-java-springboot` on port 8201.
...
- Frontend runs against `crud-be-java-springboot` with real PostgreSQL

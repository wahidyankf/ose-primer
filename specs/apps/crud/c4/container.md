...
graph TD
    ...
    subgraph CICD["CI Pipelines"]
        MAIN_CI["Main CI<br/>──────────────────<br/>typecheck, lint, test:quick<br/>Coverage via rhino-cli<br/>On push to main"]:::ci
...
| ------------------------- | ---------- | ------------ | ---------- | -------- |
| crud-be-golang-gin        | Go         | Gin          | PostgreSQL | >= 90%   |
| crud-be-java-springboot   | Java       | Spring Boot  | PostgreSQL | >= 90%   |
| crud-be-java-vertx        | Java       | Vert.x       | PostgreSQL | >= 90%   |
| crud-be-kotlin-ktor       | Kotlin     | Ktor         | PostgreSQL | >= 90%   |
| crud-be-python-fastapi    | Python     | FastAPI      | PostgreSQL | >= 90%   |
| crud-be-rust-axum         | Rust       | Axum         | PostgreSQL | >= 90%   |
| crud-be-ts-effect         | TypeScript | Effect       | PostgreSQL | >= 90%   |
| crud-be-fsharp-giraffe    | F#         | Giraffe      | PostgreSQL | >= 90%   |
| crud-be-csharp-aspnetcore | C#         | ASP.NET Core | PostgreSQL | >= 90%   |
| crud-be-clojure-pedestal  | Clojure    | Pedestal     | PostgreSQL | >= 90%   |
| crud-be-elixir-phoenix    | Elixir     | Phoenix      | PostgreSQL | >= 90%   |

### Single Page Application (3 implementations)

| App                       | Language   | Framework      | Coverage |
| ------------------------- | ---------- | -------------- | -------- |
| crud-fe-ts-nextjs         | TypeScript | Next.js 16     | >= 70%   |
| crud-fe-ts-tanstack-start | TypeScript | TanStack Start | >= 70%   |
| crud-fe-dart-flutterweb   | Dart       | Flutter Web    | >= 70%   |

### E2E Test Suites

| App         | Targets         | Scope                       |
| ----------- | --------------- | --------------------------- |
| crud-be-e2e | All 11 backends | Playwright, per-backend CI  |
| crud-fe-e2e | All 3 frontends | Playwright, per-frontend CI |

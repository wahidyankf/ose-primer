...
func (s *dartScaffoldSteps) anExistingGeneratedContractsDirWithOldScaffoldFiles() error {
	...
	return os.WriteFile(filepath.Join(libDir, "crud_contracts.dart"), []byte("// old barrel\n"), 0644)
}
...
func (s *dartScaffoldSteps) pubspecYamlIsCreatedWithCorrectContent() error {
	...
	if !strings.Contains(string(data), "name: crud_contracts") {
		return fmt.Errorf("expected pubspec.yaml to contain 'name: crud_contracts', got: %s", string(data))
	}
	return nil
}

func (s *dartScaffoldSteps) theBarrelLibraryIsCreatedWithPartDirectivesForEachModel() error {
	data, err := os.ReadFile(filepath.Join(s.srcDir, "lib", "crud_contracts.dart"))
	if err != nil {
...
func (s *dartScaffoldSteps) theExistingFilesAreOverwrittenWithFreshScaffold() error {
	...
	if !strings.Contains(string(data), "name: crud_contracts") {
		return fmt.Errorf("expected pubspec.yaml to be overwritten with 'name: crud_contracts', got: %s", string(data))
	}

	// barrel should have been overwritten
	barrelData, err := os.ReadFile(filepath.Join(s.srcDir, "lib", "crud_contracts.dart"))
	if err != nil {
		return fmt.Errorf("reading barrel library: %w", err)
	}

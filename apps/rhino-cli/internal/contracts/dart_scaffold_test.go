...
func TestScaffoldDart_WithModels(t *testing.T) {
	...
	if !strings.Contains(pubspec, "name: crud_contracts") {
		t.Error("pubspec.yaml missing 'name: crud_contracts'")
	}
	...
	barrelData, err := os.ReadFile(filepath.Join(dir, "lib", "crud_contracts.dart"))
	if err != nil {

...
// step4StageAyokoding auto-stages crud-fs-ts-nextjs content changes so they are
// included in the current commit. The crud-fs-ts-nextjs content directory may be
...
func step4StageAyokoding(gitRoot string, deps Deps) {
	cmd := deps.ExecCommand("git", "add", "apps/crud-fs-ts-nextjs/content/")
	cmd.Dir = gitRoot
	_ = cmd.Run()
}
...
//
// Some apps (e.g. crud-be-ts-effect, crud-fe-ts-nextjs) have their own package-lock.json
...
func step5bSyncLockfiles(gitRoot string, staged []string, deps Deps) error {
	...
	for _, f := range staged {
		...
		// Ensure it's a direct child of apps/ (e.g. apps/crud-be-ts-effect/package.json),

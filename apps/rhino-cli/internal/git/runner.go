...
// step4StageAyokoding auto-stages demo-fs-ts-nextjs content changes so they are
// included in the current commit. The demo-fs-ts-nextjs content directory may be
...
func step4StageAyokoding(gitRoot string, deps Deps) {
	cmd := deps.ExecCommand("git", "add", "apps/demo-fs-ts-nextjs/content/")
	cmd.Dir = gitRoot
	_ = cmd.Run()
}
...
//
// Some apps (e.g. demo-be-ts-effect, demo-fe-ts-nextjs) have their own package-lock.json

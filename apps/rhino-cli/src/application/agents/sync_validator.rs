...
use super::cursor::{
    CURSOR_AGENT_DIR, CURSOR_MIRROR_REMEDIATION, is_mirrorable_agent_filename,
...
/// Run all sync-parity checks between `.claude/agents/` and `.cursor/agents/`.
pub fn validate_cursor_sync(repo_root: &Path) -> ValidationResult {
    let start = Instant::now();
    let mut result = ValidationResult::default();

    result.tally(validate_cursor_agent_count(repo_root));

    for c in validate_cursor_equivalence(repo_root) {
        result.tally(c);
    }

    for c in validate_cursor_orphans(repo_root) {
        result.tally(c);
    }
...
/// Check that `.cursor/agents/` contains at least as many `.md` files as `.claude/agents/`.
fn validate_cursor_agent_count(repo_root: &Path) -> ValidationCheck {
    ...
    if cursor_count >= claude_count {
        ValidationCheck::passed(
            "Cursor Agent Count",
            "Cursor agents directory contains every Claude agent",
        )
        .with_expected(format!(">= {claude_count} agents"))
        .with_actual(format!("{cursor_count} agents"))
    } else {
        ValidationCheck::failed(
            "Cursor Agent Count",
            format!(">= {claude_count} agents"),
            format!("{cursor_count} agents"),
            format!("Cursor mirror missing one or more Claude agents; {CURSOR_MIRROR_REMEDIATION}"),
        )
    }
}

/// Compare each Claude agent to its Cursor counterpart for byte parity.
fn validate_cursor_equivalence(repo_root: &Path) -> Vec<ValidationCheck> {
    ...
    for (claude_path, name) in files {
        ...
        let expected = match render_cursor_agent_bytes(&claude_path) {
            ...
            Err(e) => {
                checks.push(ValidationCheck::failed_msg(
                    &check_name,
                    format!("Failed to render expected Cursor agent: {e}"),
                ));
                continue;
            }
        };

        let actual = match fs::read(&cursor_path) {
            Ok(v) => v,
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
                checks.push(ValidationCheck::failed(
                    &check_name,
                    "Cursor mirror file present",
                    "Cursor mirror file missing",
                    format!("Missing Cursor mirror for {name}; {CURSOR_MIRROR_REMEDIATION}"),
                ));
                continue;
            }
            Err(e) => {
                checks.push(ValidationCheck::failed_msg(
                    &check_name,
                    format!("Failed to read Cursor agent: {e}"),
                ));
                continue;
            }
        };
        ...
        if expected == actual {
            checks.push(ValidationCheck::passed(
                &check_name,
                "Cursor mirror matches generated content",
            ));
        } else {
            checks.push(ValidationCheck::failed(
                &check_name,
                ...
                format!("Cursor mirror drift for {name}; {CURSOR_MIRROR_REMEDIATION}"),
            ));
        }
    }
...
/// Report Cursor mirror files that have no Claude source counterpart.
fn validate_cursor_orphans(repo_root: &Path) -> Vec<ValidationCheck> {
    let mut checks = Vec::new();
    let claude_dir = repo_root.join(".claude").join("agents");
    let cursor_dir = repo_root.join(CURSOR_AGENT_DIR);

    let Ok(entries) = fs::read_dir(&cursor_dir) else {
        return checks;
    };
    ...
    for entry in entries.flatten() {
        ...
        if !claude_path.exists() {
            checks.push(ValidationCheck::failed(
                ...
                format!("{name} has no Claude counterpart"),
                format!("Stale Cursor mirror file {name}; delete it or restore the Claude source"),
            ));
        }
    }
...
fn count_markdown_files(dir: &Path) -> usize {
    ...
    for entry in entries.flatten() {
        let name = entry.file_name().to_string_lossy().into_owned();
        if entry.file_type().is_ok_and(|t| !t.is_dir())
            && is_mirrorable_agent_filename(&name, false)
        {
            count += 1;
        }
    }

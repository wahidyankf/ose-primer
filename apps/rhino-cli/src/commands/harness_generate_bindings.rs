//! `harness generate-bindings` — runs `OpenCode` sync then Amazon Q emit-bindings.
//!
//! Combines `harness sync` (`OpenCode`) and `harness emit-bindings` (Amazon Q)
//! into a single idempotent command.  Use `--opencode=false` or
//! `--amazonq=false` to skip individual steps.
...
/// CLI arguments for `harness generate-bindings`.
#[derive(Args, Debug)]

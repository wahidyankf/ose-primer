...
pub fn run_at_root(repo_root: &Path, writer: &mut dyn Write) -> Result<(), Error> {
    ...
    validate_rust_toolchain_lint_components(repo_root, writer)?;
...
const REQUIRED_RUST_TOOLCHAIN_COMPONENTS: [&str; 2] = ["rustfmt", "clippy"];

/// Validates that every `rust-toolchain.toml` declares the lint components.
///
/// A crate that pins a `channel` without listing `components` relies on that
/// exact toolchain having been installed with a non-minimal profile, or on a
/// sibling crate's toolchain file having added the components first. CI
/// pre-installs each declared MSRV with `--profile minimal`, and `nx run-many`
/// runs the per-crate lint targets in parallel, so the component add and the
/// `cargo fmt --check` that needs it race: the gate fails with
/// `'cargo-fmt' is not installed for the toolchain '<channel>'` on some runs
/// and passes on others. Declaring the components makes rustup provision them
/// before the first proxy call in that directory, on every runner.
///
/// # Errors
///
/// Returns an error when a `rust-toolchain.toml` omits a required component,
/// or when the diagnostic cannot be written.
fn validate_rust_toolchain_lint_components(
    repo_root: &Path,
    ...
    for relative in rust_toolchain_manifests(repo_root) {
        ...
        let declared = declared_toolchain_components(&contents);
...
/// directories are considered — the same locations the CI MSRV pre-install
/// step scans for `Cargo.toml` floors. Vendored copies under `target/`,
/// `node_modules/`, or a local cargo home are deliberately out of scope.
fn rust_toolchain_manifests(repo_root: &Path) -> Vec<String> {
...
/// Extracts the component names from a `rust-toolchain.toml` body.
///
/// Handles both the single-line and multi-line array forms. Returns an empty
/// vector when the file declares no `components` key at all.
fn declared_toolchain_components(contents: &str) -> Vec<String> {
    let Some(after_key) = contents.split_once("components") else {
        return Vec::new();
    };
    let Some(open) = after_key.1.split_once('[') else {
        return Vec::new();
    };
    let Some((body, _)) = open.1.split_once(']') else {
        return Vec::new();
    };
    body.split(',')
...
/// Binds the Gherkin scenario "A pinned Rust toolchain without lint components
/// fails validation"
/// (specs/apps/rhino/behavior/rhino-cli/gherkin/gate/gate-validation.feature).
#[cfg(test)]
#[allow(clippy::unwrap_used, clippy::panic)]
#[test]
fn rust_toolchain_without_lint_components_fails() {
    let repo = tempfile::TempDir::new().unwrap();
    ...
    let mut output = Vec::new();
    let result = validate_rust_toolchain_lint_components(repo.path(), &mut output);
    let rendered = String::from_utf8_lossy(&output);
    assert!(
        result.is_err()
            && rendered.contains("apps/coralpolyp-be/rust-toolchain.toml")
            && rendered.contains("rustfmt")
            && rendered.contains("clippy"),
        "a channel-only rust-toolchain.toml must fail validation and name the missing components; \
         result_ok={}, output={rendered:?}",
        result.is_ok()
    );
}

/// Binds the Gherkin scenario "A pinned Rust toolchain declaring the lint
/// components passes validation"
/// (specs/apps/rhino/behavior/rhino-cli/gherkin/gate/gate-validation.feature).
#[cfg(test)]
#[allow(clippy::unwrap_used, clippy::panic)]
#[test]
fn rust_toolchain_with_lint_components_passes() {
    let repo = tempfile::TempDir::new().unwrap();
    ...
    assert!(
        validate_rust_toolchain_lint_components(repo.path(), &mut Vec::new()).is_ok(),
        "rust-toolchain.toml files declaring rustfmt and clippy must pass validation"
    );
}
...
#[allow(clippy::unwrap_used, clippy::panic)]
#[test]
fn rust_toolchain_components_parse_from_multi_line_array() {
    let contents = concat!(
        "[toolchain]\n",
        "channel = \"1.95.0\"\n",
        "components = [\n",
    ...
    assert_eq!(
        declared_toolchain_components(contents),

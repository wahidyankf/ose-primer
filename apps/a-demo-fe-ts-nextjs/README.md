...
[demo-be API](../a-demo-be-golang-gin/README.md).
...
nx run a-demo-fe-ts-nextjs:start

# Type checking
nx typecheck a-demo-fe-ts-nextjs

# Lint code (oxlint)
nx lint a-demo-fe-ts-nextjs

# Fast quality gate: unit tests + coverage check + specs coverage check
nx run a-demo-fe-ts-nextjs:test:quick

# Unit tests only
nx run a-demo-fe-ts-nextjs:test:unit
```
...
| ----- | --------------------------- | ------------ | -------------------------------------- | ------- |
| Unit  | @amiceli/vitest-cucumber    | All mocked   | `nx run a-demo-fe-ts-nextjs:test:unit` | Yes     |
...
```bash
docker build -t a-demo-fe-ts-nextjs:latest apps/a-demo-fe-ts-nextjs/
```

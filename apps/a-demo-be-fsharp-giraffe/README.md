...
This application mirrors the same REST API contract as `a-demo-be-golang-gin` (Go/Gin) and
`a-demo-be-elixir-phoenix` (Elixir/Phoenix), providing an F#/Giraffe alternative implementation.
...
| ----------------- | -------------- | ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DATABASE_URL`    | Yes (non-test) | —                                                | PostgreSQL connection string (e.g., `Host=localhost;Database=a_demo_be_fsharp_giraffe;Username=a_demo_be_fsharp_giraffe;Password=a_demo_be_fsharp_giraffe`) |
...
`infra/dev/a-demo-be-fsharp-giraffe/`. It will start PostgreSQL 17 and the F#/Giraffe application with
...
| E2E         | (a-demo-be-e2e)    | Playwright                                  | PostgreSQL 17 (real) | Full HTTP against running server                               | Yes (port 8201)           |
...
All BDD tests share the same Gherkin feature files from `specs/apps/a-demo/be/gherkin/` as
`a-demo-be-golang-gin`, `a-demo-be-elixir-phoenix`, and other backend implementations.
...
- [a-demo-be-e2e](../a-demo-be-e2e/README.md) — Shared E2E tests

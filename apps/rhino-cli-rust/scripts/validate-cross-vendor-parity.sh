...
# Build the Rust binary once up front (release profile, matching the Nx build
# target). The audit invariants invoke it directly afterwards.
echo "validate-cross-vendor-parity: building Rust binary…" >&2
( cd "${RS_DIR}" && cargo build --release --quiet ) \
  || { echo "validate-cross-vendor-parity: Rust build failed" >&2; exit 2; }
[[ -x "${RS_BIN}" ]] || { echo "validate-cross-vendor-parity: Rust binary missing at ${RS_BIN}" >&2; exit 2; }

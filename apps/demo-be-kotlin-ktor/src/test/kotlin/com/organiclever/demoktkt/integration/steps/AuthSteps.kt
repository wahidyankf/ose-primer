...
class AuthSteps {
  ...
  private fun loginAs(username: String, password: String): Boolean {
    ...
    if (status == 200) {
      ...
      JsonHelper.getString(body, "refresh_token")?.let { TestWorld.refreshTokens[username] = it }
      return true
    }
    return false
  }
    ...
    runBlocking {
      ...
      val user =
        if (existing == null) {
          ...
          TestWorld.userIds[username] = u.id.toString()
          u
        } else {
          TestWorld.userIds[username] = existing.id.toString()
          existing
  ...
  fun aliceHasAlreadyLoggedOutOnce() {
    ...
    HttpHelper.post("/api/v1/auth/logout", "", token)
  }
  ...
  fun theAdminHasDisabledAlicesAccount() {
    ...
    HttpHelper.post("/api/v1/admin/users/$aliceId/disable", """{"reason":"test"}""", adminToken)
  }

  @Given("alice has deactivated her own account via POST \\/api\\/v1\\/users\\/me\\/deactivate")
  fun aliceHasDeactivatedHerOwnAccount() {
    val token = TestWorld.accessTokens["alice"] ?: error("alice has no access token")
    HttpHelper.post("/api/v1/users/me/deactivate", "", token)
  }
  ...
  fun theClientSendsPostRegisterWithBody(username: String, email: String, password: String) {
    ...
    val (status, respBody) = HttpHelper.post("/api/v1/auth/register", body)
    TestWorld.lastResponseStatus = status
    TestWorld.lastResponseBody = respBody
    if (status == 201) {
      JsonHelper.getString(respBody, "id")?.let { TestWorld.userIds[username] = it }
    }
  }
  ...
  fun theClientSendsPostLoginWithBody(username: String, password: String) {
    ...
    val (status, respBody) = HttpHelper.post("/api/v1/auth/login", body)
    TestWorld.lastResponseStatus = status
    TestWorld.lastResponseBody = respBody
  }
  ...
  fun aliceSendsPostRefreshWithRefreshToken() {
    val refreshToken = TestWorld.refreshTokens["alice"] ?: error("alice has no refresh token")
    val accessToken = TestWorld.accessTokens["alice"]
    val body = """{"refresh_token":"$refreshToken"}"""
    val (status, respBody) = HttpHelper.post("/api/v1/auth/refresh", body, accessToken)
    TestWorld.lastResponseStatus = status
    TestWorld.lastResponseBody = respBody
  }
  ...
  fun aliceSendsPostLogoutWithAccessToken() {
    val token = TestWorld.accessTokens["alice"] ?: error("alice has no access token")
    val (status, respBody) = HttpHelper.post("/api/v1/auth/logout", "", token)
    TestWorld.lastResponseStatus = status
    TestWorld.lastResponseBody = respBody
  }
  ...
  fun aliceSendsPostLogoutAllWithAccessToken() {
    val token = TestWorld.accessTokens["alice"] ?: error("alice has no access token")
    val (status, respBody) = HttpHelper.post("/api/v1/auth/logout-all", "", token)
    TestWorld.lastResponseStatus = status
    TestWorld.lastResponseBody = respBody
  }
  ...
  fun theClientSendsPostExpensesWithBody(
    ...
    val (status, respBody) = HttpHelper.post("/api/v1/expenses", body)
    TestWorld.lastResponseStatus = status
    TestWorld.lastResponseBody = respBody
  }
}

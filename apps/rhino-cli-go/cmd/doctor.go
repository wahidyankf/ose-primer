...
Tools checked:
  ...
  java           — from apps/crud-be-fsharp-giraffe-jasb/pom.xml → <java.version>
  maven          — any version (no config file)
  golang         — from apps/rhino-cli/go.mod → go directive
  python         — from apps/crud-be-python-fastapi/.python-version
  rust           — from apps/crud-be-rust-axum/Cargo.toml → rust-version
  cargo-llvm-cov — any version (cargo subcommand)
  elixir         — from .tool-versions → elixir
  erlang         — from .tool-versions → erlang
  dotnet         — from apps/crud-be-fsharp-giraffe/global.json → sdk.version
  clojure        — any version (no config file)
  dart           — from apps/crud-fe-dart-flutterweb/pubspec.yaml → environment.sdk
  flutter        — from apps/crud-fe-dart-flutterweb/pubspec.yaml → environment.flutter

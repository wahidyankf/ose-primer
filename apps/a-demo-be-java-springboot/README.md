...
to `infra/dev/a-demo-be-java-springboot/.env` for local development.
...
1. Edit source files in `apps/a-demo-be-java-springboot/src/`
...
nx dev a-demo-be-java-springboot
...
```bash
docker build -t a-demo-be-java-springboot:latest apps/a-demo-be-java-springboot/
```

Run the image:

```bash
docker run --rm -p 8201:8201 a-demo-be-java-springboot:latest
```
...
```bash
# Build JAR
nx build a-demo-be-java-springboot
...
Output: `target/a-demo-be-java-springboot-1.0.0.jar`

Run:

```bash
java -XX:+UseZGC -jar apps/a-demo-be-java-springboot/target/a-demo-be-java-springboot-1.0.0.jar --spring.profiles.active=prod
```
...
nx run a-demo-be-java-springboot:start

# Run fast quality gate: unit tests + coverage check (no running service needed)
nx run a-demo-be-java-springboot:test:quick

# Run unit tests only (Cucumber JVM + mocked repos, no Spring context)
nx run a-demo-be-java-springboot:test:unit

# Run integration tests (Cucumber JVM + real PostgreSQL via docker-compose)
nx run a-demo-be-java-springboot:test:integration

# Lint code
nx lint a-demo-be-java-springboot
...
# Edit code in apps/a-demo-be-java-springboot/src/
...
nx run a-demo-be-java-springboot:test:unit        # mvn test

# Integration tests (Cucumber JVM + real PostgreSQL via docker-compose)
nx run a-demo-be-java-springboot:test:integration # docker compose up --abort-on-container-exit
```
...
The Docker-based development environment uses a custom image built from `infra/dev/a-demo-be-java-springboot/Dockerfile.be.dev`:
...
- ✅ **Isolated**: Maven installation contained to a-demo-be-java-springboot service only
...
Three levels of testing consume the shared Gherkin scenarios from [`specs/apps/a-demo/be/gherkin/`](../../specs/apps/a-demo/be/gherkin/README.md):

| Level       | Tool                           | Dependencies         | Command                                             | Cached? |
| ----------- | ------------------------------ | -------------------- | --------------------------------------------------- | ------- |
| Unit        | Cucumber JVM + mocked repos    | InMemoryDataStore    | `nx run a-demo-be-java-springboot:test:unit`        | Yes     |
| Integration | Cucumber JVM + real PostgreSQL | Docker Compose       | `nx run a-demo-be-java-springboot:test:integration` | No      |
| E2E         | Playwright + HTTP              | Running backend + DB | `nx run a-demo-be-e2e:test:e2e`                     | No      |
...
```bash
nx run a-demo-be-java-springboot:test:unit
# or: cd apps/a-demo-be-java-springboot && mvn test
```
...
```bash
nx run a-demo-be-java-springboot:test:integration
# or: cd apps/a-demo-be-java-springboot && docker compose -f docker-compose.integration.yml down -v && docker compose -f docker-compose.integration.yml up --abort-on-container-exit --build
```

### E2E Testing

The [`a-demo-be-e2e`](../a-demo-be-e2e/) project provides Playwright-based E2E tests

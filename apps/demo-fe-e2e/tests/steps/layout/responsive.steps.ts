...
Given("the viewport is set to {string} \\(1280x800)", async ({ page }, _name: string) => {
  await page.setViewportSize({ width: 1280, height: 800 });
});

Given("the viewport is set to {string} \\(768x1024)", async ({ page }, _name: string) => {
  await page.setViewportSize({ width: 768, height: 1024 });
});

Given("the viewport is set to {string} \\(375x667)", async ({ page }, _name: string) => {
  await page.setViewportSize({ width: 375, height: 667 });
});

When("{word} taps the hamburger menu button", async ({ page }) => {
  await page
    .getByRole("button", { name: /menu|hamburger|toggle/i })
    .first()
    .click();
});
...
Then("entries should be displayed as stacked cards", async ({ page }) => {
  await expect(
    page.getByTestId("entry-card").or(page.getByRole("article")).or(page.locator(".card")).first(),
  ).toBeVisible();
});

...
const feature = await loadFeature(
  path.resolve(process.cwd(), "../../specs/apps/crud/behavior/crud-web/gherkin/expenses/currency-handling.feature"),
);
...
describeFeature(feature, ({ Scenario, Background }) => {
  ...
  Scenario("USD expense displays two decimal places", ({ Given, When, Then, And }) => {
    ...
    When('alice views the entry detail for "Coffee"', async () => {
      ...
      const ExpenseDetailPage = (await import("@/app/(dashboard)/expenses/[id]/page")).default;
      render(
  ...
  Scenario("Unsupported currency code shows a validation error", ({ When, Then, And }) => {
    When("alice navigates to the new entry form", async () => {
      ...
      const ExpensesPage = (await import("@/app/(dashboard)/expenses/page")).default;
      render(
        <QueryClientProvider client={queryClient}>
          <ExpensesPage />
        </QueryClientProvider>,
  ...
  Scenario("Expense summary groups totals by currency", ({ Given, When, Then, And }) => {
    ...
    When("alice navigates to the expense summary page", async () => {
      const SummaryPage = (await import("@/app/(dashboard)/expenses/summary/page")).default;
      render(
        <QueryClientProvider client={queryClient}>
          <SummaryPage />
        </QueryClientProvider>,

...
import com.demobejavx.unit.UnitFactory;
...
public class UnitAttachmentSteps {
    ...
    public UnitAttachmentSteps(ScenarioState state) {
        this.state = state;
    }
    ...
    public void aliceUploadsFileToExpense(String filename, String contentType) throws Exception {
        ...
        ServiceResponse response = svc().uploadAttachment(token, expenseId, filename, contentType,
                content);
        state.setLastResponse(response);
        if (response.statusCode() == 201 && response.body() != null) {
            state.setAttachmentId(response.body().getString("id"));
        }
    }
    ...
    public void aliceUploadsOversizedFile() throws Exception {
        ...
        ServiceResponse response = svc().uploadAttachment(token, expenseId, "big.jpg",
    ...
    public void aliceSendsGetAttachments() throws Exception {
        ...
        ServiceResponse response = svc().listAttachments(token, expenseId);
        state.setLastResponse(response);
    }
    ...
    public void aliceSendsDeleteAttachment() throws Exception {
        ...
        ServiceResponse response = svc().deleteAttachment(token, expenseId, attachmentId);
        state.setLastResponse(response);
    }
    ...
    public void aliceSendsDeleteNonExistentAttachment() throws Exception {
        ...
        ServiceResponse response = svc().deleteAttachment(token, expenseId, "99999");
        state.setLastResponse(response);
    }
    ...
    public void aliceHasUploadedFileToEntry(String filename, String contentType) throws Exception {
        ...
        if (resp != null && resp.statusCode() == 201 && resp.body() != null) {
            state.setAttachmentId(resp.body().getString("id"));
        }
    }
    ...
    public void bobHasCreatedEntry(String amount, String currency, String category,
        ...
        ServiceResponse resp = svc().createExpense(bobToken, amount, currency, category,
                description, date, type);
        Assertions.assertEquals(201, resp.statusCode(),
                "Expected 201 creating bob's entry but got " + resp.statusCode());
        state.setBobExpenseId(resp.body().getString("id"));
    }

...
# Typecheck
nx run oseplatform-fs:typecheck

# Lint (oxlint)
nx run oseplatform-fs:lint

# Unit tests + coverage + links
nx run oseplatform-fs:test:quick

# Integration tests
nx run oseplatform-fs:test:integration
```
...
Deployed to Vercel via production branch `prod-oseplatform-fs`.

- **Production**: <https://oseplatform.com>
- **Deploy**: Push `main` to `prod-oseplatform-fs`; Vercel builds automatically

```bash
git push origin main:prod-oseplatform-fs
```

## Related

- [Main Repository](https://github.com/wahidyankf/open-sharia-enterprise)
- [apps-oseplatform-fs-deployer](../../.claude/agents/) - AI agent for deployments

...
- **deploy-ayokoding-fs** — Diffs `main` against `prod-ayokoding-fs`, builds Hugo site and force-pushes to production branch if changes detected (Vercel picks up the push)
- **deploy-oseplatform-fs** — Same change-detection and deploy pattern for oseplatform.com

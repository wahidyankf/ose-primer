...
public class UnitAuthSteps {
    ...
    public void theClientSendsGetUsersMeWithAlicesToken() {
        ...
        org.springframework.http.ResponseEntity<com.demobejasb.user.dto.UserProfileResponse>
    ...
    public void theAdminSendsPostUnlockAliceShared() {
        ...
        try {
            org.springframework.http.ResponseEntity<com.demobejasb.admin.dto.AdminUserResponse>
    ...
    Object resolveField(final Object body, final String jsonField) {
        ...
        if (body instanceof com.demobejasb.user.dto.UserProfileResponse resp) {
            return switch (jsonField) {
                case "id" -> resp.id();
                case "username" -> resp.username();
        ...
        if (body instanceof com.demobejasb.admin.dto.AdminUserResponse resp) {
            return switch (jsonField) {
                case "id" -> resp.id();
                case "username" -> resp.username();
                case "email" -> resp.email();
        ...
        if (body instanceof com.demobejasb.expense.dto.ExpenseResponse resp) {
            return switch (jsonField) {
                case "id" -> resp.id();
        ...
        if (body instanceof com.demobejasb.report.dto.PlReportResponse resp) {
            return switch (jsonField) {
                case "totalIncome" -> resp.totalIncome();
                case "totalExpense" -> resp.totalExpense();
                case "net" -> resp.net();
                case "incomeBreakdown" -> resp.incomeBreakdown();
                case "expenseBreakdown" -> resp.expenseBreakdown();
                default -> null;
            };
        }

...
This is an alternative implementation of `a-demo-be-golang-gin` (Go/Gin), built with
...
docker compose -f infra/dev/a-demo-be-elixir-phoenix/docker-compose.yml up --build
```
...
```bash
nx run a-demo-be-elixir-phoenix:install          # mix deps.get
nx run a-demo-be-elixir-phoenix:dev              # mix phx.server (development)
nx run a-demo-be-elixir-phoenix:test:quick       # coveralls.lcov (unit only) + rhino-cli coverage gate (>=90%)
nx run a-demo-be-elixir-phoenix:test:unit        # mix test --only unit (no coverage measurement)
nx run a-demo-be-elixir-phoenix:test:integration # docker compose: real PostgreSQL + all BDD scenarios
nx run a-demo-be-elixir-phoenix:lint             # mix credo --strict
nx run a-demo-be-elixir-phoenix:typecheck        # mix compile (warnings-as-errors; depends on codegen)
nx run a-demo-be-elixir-phoenix:build            # mix compile (prod, warnings-as-errors; depends on codegen)
```
...
e2e         → Playwright against a live running stack (apps/a-demo-be-e2e)
```
...
nx run a-demo-be-elixir-phoenix:test:unit

# Pre-push quality gate — run unit tests with coverage + enforce >=90%
nx run a-demo-be-elixir-phoenix:test:quick
```
...
- Controller error-path tests in `test/a_demo_be_exph_web/controllers/coverage_test.exs`
...
- `AADemoBeExph.Accounts` → `AADemoBeExph.InMemoryAccounts`
- `AADemoBeExph.Token.TokenContext` → `AADemoBeExph.InMemoryTokenContext`
- `AADemoBeExph.Expense.ExpenseContext` → `AADemoBeExph.InMemoryExpenseContext`
- `AADemoBeExph.Attachment.AttachmentContext` → `AADemoBeExph.InMemoryAttachmentContext`
...
```bash
nx run a-demo-be-elixir-phoenix:test:integration
```
...
End-to-end tests live in `apps/a-demo-be-e2e` and run Playwright scenarios against a fully
deployed stack. See that project's README for details.

## BDD Feature Specifications

Feature specifications live in `specs/apps/a-demo/be/gherkin/` (workspace root) and are shared

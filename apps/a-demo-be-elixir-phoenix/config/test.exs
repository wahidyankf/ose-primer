...
config :a_demo_be_exph, AADemoBeExph.Auth.Guardian,
  secret_key: "test_secret_do_not_use_in_production",
  ttl: {24, :hours}

# In-memory module overrides — no PostgreSQL required for tests
config :a_demo_be_exph, :accounts_module, AADemoBeExph.Test.InMemoryAccounts
config :a_demo_be_exph, :token_module, AADemoBeExph.Test.InMemoryTokenContext
config :a_demo_be_exph, :expense_module, AADemoBeExph.Test.InMemoryExpenseContext
config :a_demo_be_exph, :attachment_module, AADemoBeExph.Test.InMemoryAttachmentContext

# Elixir Cabbage BDD — feature files relative to workspace root
config :elixir_cabbage,
  features: Path.expand("../../../specs/apps/a-demo/be/gherkin/", __DIR__) <> "/"

...
```bash
nx dev a-demo-be-clojure-pedestal
```
...
```bash
export DATABASE_URL="jdbc:postgresql://localhost:5432/a_demo_be_cjpd"
export DB_USER="a_demo_be_cjpd"
export DB_PASSWORD="a_demo_be_cjpd"
```
...
- `test/a_demo_be_cjpd/**/*_test.clj` — clojure.test namespaces
...
```bash
nx run a-demo-be-clojure-pedestal:test:quick
```
...
```bash
nx run a-demo-be-clojure-pedestal:test:integration
```
...
The `a-demo-be-cjpd.main` namespace is excluded from coverage (entry point only).
...
```bash
nx run a-demo-be-clojure-pedestal:test:quick
nx run a-demo-be-clojure-pedestal:test:unit
nx run a-demo-be-clojure-pedestal:test:integration
nx build a-demo-be-clojure-pedestal
nx run a-demo-be-clojure-pedestal:typecheck
```

...
End-to-end tests for all [demo-fe frontends](../../specs/apps/a-demo/fe/README.md),
...
Feature files in [`specs/apps/a-demo/fe/gherkin/`](../../specs/apps/a-demo/fe/gherkin/README.md) are the source of truth:
...
```
specs/apps/a-demo/fe/gherkin/**/*.feature    <- source of truth (read-only)
        |
...
| --------------------- | ----------------------- | ------------ |
| `a-demo-fe-ts-nextjs` | Next.js 16 (App Router) | 3301         |
...
```bash
nx install a-demo-fe-e2e
cd apps/a-demo-fe-e2e && npx playwright install --with-deps chromium && cd ../..
```
...
nx run a-demo-fe-e2e:test:e2e:ui

# View HTML report from last run
nx run a-demo-fe-e2e:test:e2e:report

# Generate spec files only (without running tests)
cd apps/a-demo-fe-e2e && npx bddgen

# Lint TypeScript source files (oxlint)
nx run a-demo-fe-e2e:lint

# Type check
nx typecheck a-demo-fe-e2e
...
1. **`test-a-demo-fe-ts-nextjs.yml`** — the dedicated frontend workflow. Triggers on changes to `a-demo-fe-ts-nextjs`, `a-demo-fe-e2e`, or `a-demo-be-golang-gin`. Always uses the Go/Gin backend stack via `infra/dev/a-demo-fe-ts-nextjs/docker-compose.yml`.

2. **Each `test-a-demo-be-*.yml` backend workflow** — after the backend's integration and BE E2E tests pass, an `e2e-fe` job runs on a fresh runner. It starts the **same backend** being tested (with `ENABLE_TEST_API=true`) alongside the Next.js frontend via that backend's `infra/dev/a-demo-be-<name>/docker-compose.ci.yml` overlay. This validates that the frontend works end-to-end against every supported backend — not just Go/Gin.

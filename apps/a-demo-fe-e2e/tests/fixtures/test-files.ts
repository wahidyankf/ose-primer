import * as fs from "fs";
import * as path from "path";
import { fileURLToPath } from "url";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

const FIXTURES_DIR = path.join(__dirname);

// Minimal valid JPEG (1x1 pixel)
const MINIMAL_JPEG = Buffer.from([
  ,
]);

// Minimal valid PDF
const MINIMAL_PDF = Buffer.from(
  `%PDF-1.4
1 0 obj<</Type/Catalog/Pages 2 0 R>>endobj
2 0 obj<</Type/Pages/Kids[3 0 R]/Count 1>>endobj
3 0 obj<</Type/Page/MediaBox[0 0 612 792]/Parent 2 0 R>>endobj
xref
0 4
0000000000 65535 f
0000000009 00000 n
0000000052 00000 n
0000000101 00000 n
trailer<</Size 4/Root 1 0 R>>
startxref
171
%%EOF`,
);

export function getReceiptJpgPath(): string {
  const filePath = path.join(FIXTURES_DIR, "receipt.jpg");
  if (!fs.existsSync(filePath)) {
    fs.writeFileSync(filePath, MINIMAL_JPEG);
  }
  return filePath;
}

export function getInvoicePdfPath(): string {
  const filePath = path.join(FIXTURES_DIR, "invoice.pdf");
  if (!fs.existsSync(filePath)) {
    fs.writeFileSync(filePath, MINIMAL_PDF);
  }
  return filePath;
}

export function getOversizedFilePath(): string {
  const filePath = path.join(FIXTURES_DIR, "oversized.jpg");
  if (!fs.existsSync(filePath)) {
    // Create a ~15MB file that exceeds typical limits
    const size = 15 * 1024 * 1024;
    const buf = Buffer.alloc(size, 0xff);
    // Write JPEG header
    MINIMAL_JPEG.copy(buf, 0);
    fs.writeFileSync(filePath, buf);
  }
  return filePath;
}

export function getMalwareExePath(): string {
  const filePath = path.join(FIXTURES_DIR, "malware.exe");
  if (!fs.existsSync(filePath)) {
    fs.writeFileSync(filePath, Buffer.from("MZ fake exe content"));
  }
  return filePath;
}

...
1. Edit source files in `apps/crud-be-java-springboot/src/`
...
Edit `apps/crud-be-java-springboot/src/main/java/com/demo/be/hello/controller/HelloController.java`:
...
```bash
docker build -t crud-be-java-springboot:latest apps/crud-be-java-springboot/
```

Run the image:

```bash
docker run --rm -p 8201:8201 crud-be-java-springboot:latest
```
...
Output: `target/crud-be-java-springboot-1.0.0.jar`

Run:

```bash
java -XX:+UseZGC -jar apps/crud-be-java-springboot/target/crud-be-java-springboot-1.0.0.jar --spring.profiles.active=prod
```
...
**See**: [Nx Target Standards](../../repo-governance/development/infra/nx-targets.md) for canonical target names. Use `dev` (not `serve`) for the development server.
...
docker-compose logs -f crud-be-java-springboot
```

**Force restart**:

```bash
docker-compose restart crud-be-java-springboot
```
...
# Edit code in apps/crud-be-java-springboot/src/
...
The Docker-based development environment uses a custom image built from `infra/dev/crud-be-java-springboot/Dockerfile.be.dev`:
...
- ✅ **Isolated**: Maven installation contained to crud-be-java-springboot service only
...
Three levels of testing consume the shared Gherkin scenarios from [`specs/apps/crud/be/gherkin/`](../../specs/apps/crud/be/gherkin/README.md):
...
The [`crud-be-e2e`](../crud-be-e2e/) project provides Playwright-based E2E tests

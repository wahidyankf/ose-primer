...
Python/FastAPI implementation of the demo backend REST API — a functional twin of `a-demo-be-golang-gin`,
`a-demo-be-elixir-phoenix`, and `a-demo-be-fsharp-giraffe`.
...
```bash
cd apps/a-demo-be-python-fastapi
uv sync
```
...
| `APP_JWT_ISSUER`            | `a-demo-be-python-fastapi` | JWT issuer claim                      |
...
```bash
uv run uvicorn a_demo_be_python_fastapi.main:app --reload --port 8201
```

Or via Nx:

```bash
nx dev a-demo-be-python-fastapi
```

### With Docker Compose (PostgreSQL)

```bash
cd infra/dev/a-demo-be-python-fastapi
...
| Target                                             | Description                                              |
| -------------------------------------------------- | -------------------------------------------------------- |
| `nx build a-demo-be-python-fastapi`                | Build distributable wheel (depends on codegen)           |
| `nx dev a-demo-be-python-fastapi`                  | Start dev server with reload                             |
| `nx start a-demo-be-python-fastapi`                | Start production server                                  |
| `nx run a-demo-be-python-fastapi:test:quick`       | Unit tests + coverage check (no lint, no integration)    |
| `nx run a-demo-be-python-fastapi:test:unit`        | Unit tests only (SQLite in-memory, no external services) |
| `nx run a-demo-be-python-fastapi:test:integration` | Integration tests via Docker Compose (real PostgreSQL)   |
| `nx lint a-demo-be-python-fastapi`                 | Ruff lint check                                          |
| `nx run a-demo-be-python-fastapi:typecheck`        | Pyright type check (depends on codegen)                  |
...
### Level 3: Docker integration tests (`nx run a-demo-be-python-fastapi:test:integration`)
...
nx run a-demo-be-python-fastapi:test:quick

# Unit tests only
nx run a-demo-be-python-fastapi:test:unit

# Full Docker integration tests (requires Docker)
nx run a-demo-be-python-fastapi:test:integration
...
Tests consume the shared [`specs/apps/a-demo/be/gherkin/`](../../specs/apps/a-demo/be/gherkin/README.md)

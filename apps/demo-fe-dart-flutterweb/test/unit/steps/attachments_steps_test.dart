...
void main() {
  ...
  describeFeature('../../specs/apps/demo/fe/gherkin/expenses/attachments.feature', (
    feature,
  ) {
    feature.scenario('Uploading a JPEG image adds it to the attachment list', (
      ...
      s.then('the attachment list should contain "receipt.jpg"', () async {
    ...
    feature.scenario('Uploading a PDF document adds it to the attachment list', (
      s,
    ) {
      late String expenseId;
      late String attachmentId;
      ...
      s.then('the attachment list should contain "invoice.pdf"', () async {
        final attachments = await svc.listAttachments(expenseId);
      ...
      s.and(
        ...
        () async {
          final attachments = await svc.listAttachments(expenseId);
          final invoice = attachments.firstWhere((a) => a.id == attachmentId);
          expect(invoice.contentType, equals('application/pdf'));
        },
      );
    });

    feature.scenario('Entry detail shows all uploaded attachments', (s) {
      late String expenseId;
      ...
      s.and('the attachment list should include "receipt.jpg"', () async {
        final attachments = await svc.listAttachments(expenseId);
        expect(attachments.any((a) => a.filename == 'receipt.jpg'), isTrue);
      });
    ...
    feature.scenario('Deleting an attachment removes it from the list', (s) {
      ...
      s.then('the attachment list should not contain "receipt.jpg"', () async {
        final attachments = await svc.listAttachments(expenseId);
        expect(attachments.any((a) => a.filename == 'receipt.jpg'), isFalse);
      });
    });

    feature.scenario('Uploading an unsupported file type shows an error', (s) {
      late String expenseId;
    ...
    feature.scenario("Cannot upload attachment to another user's entry", (s) {
    ...
    feature.scenario("Cannot view attachments on another user's entry", (s) {
      late String bobExpenseId;
    ...
    feature.scenario("Cannot delete attachment on another user's entry", (s) {
      late String bobExpenseId;
      ...
      s.and('a user "bob" has created an entry with an attachment', () async {
        await svc.logout();
        svc.seedUser(
          username: 'bob',
          email: 'bob@example.com',

...
void render(Element parent) {
  ...
  filterForm.addEventListener(
    ...
    ((Event e) {
      e.preventDefault();
      final startDate = startInput.value.isNotEmpty
          ? startInput.value
          : firstOfMonth;
      final endDate = endInput.value.isNotEmpty ? endInput.value : today;
      final currency = currencySelect.value.isNotEmpty
      ...
      () async {
        try {
          final report = await report_svc.getPLReport(
            startDate,
            endDate,
            currency,
          );
...
void _renderReport(Element container, PLReport report, String currency) {
  ...
  if (report.incomeBreakdown.isNotEmpty) {
    ...
    container.appendChild(_buildBreakdownTable(report.incomeBreakdown));
  }
  ...
  if (report.expenseBreakdown.isNotEmpty) {
    ...
    container.appendChild(_buildBreakdownTable(report.expenseBreakdown));
  }
}

...
void _renderAccessTokenCard(HTMLElement parent) {
  ...
  dl.appendChild(
    _buildDlRow('Subject (User ID)', parsed.sub, ddTestId: 'token-subject'),
  );
  dl.appendChild(_buildDlRow('Issuer', parsed.iss));
  dl.appendChild(_buildDlRow('Issued At', _formatTimestamp(parsed.iat)));
  dl.appendChild(_buildDlRow('Expires At', _formatTimestamp(parsed.exp)));
  dl.appendChild(_buildDlRow('Roles', parsed.roles.join(', ')));
...
HTMLDivElement _buildDlRow(String term, String definition, {String? ddTestId}) {
...
void _renderJwksCard(HTMLElement parent) {
  ...
  token_svc
      ...
      .then((jwks) {
        ...
        endpointLine.appendChild(
          document.createTextNode('JWKS endpoint: ') as Node,
        );

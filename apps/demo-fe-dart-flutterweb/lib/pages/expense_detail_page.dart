...
void render(Element parent, String id) {
  ...
  Future.wait([expense_svc.getExpense(id), user_svc.getCurrentUser()])
      .then((results) {
        loading.remove();
...
void _renderDetail(
  ...
  editBtn.addEventListener(
    ...
    ((Event _) {
      editOpen = !editOpen;
      editFormContainer.style.setProperty(
        'display',
        editOpen ? 'block' : 'none',
      );
  ...
  final saveBtn =
      editForm.querySelector('#edit-save-btn') as HTMLButtonElement?;
  final cancelBtn =
      editForm.querySelector('#edit-cancel-btn') as HTMLButtonElement?;
  ...
  saveBtn?.addEventListener(
    ...
    ((Event _) {
      () async {
        final amountVal =
            (editForm.querySelector('#edit-amount') as HTMLInputElement?)
                ?.value ??
            '';
        final currencyVal =
            (editForm.querySelector('#edit-currency') as HTMLInputElement?)
                ?.value ??
            '';
        final typeVal =
            (editForm.querySelector('#edit-type') as HTMLInputElement?)
                ?.value ??
            '';
        final categoryVal =
            (editForm.querySelector('#edit-category') as HTMLInputElement?)
                ?.value ??
            '';
        final dateVal =
            (editForm.querySelector('#edit-date') as HTMLInputElement?)
                ?.value ??
            '';
        final descriptionVal =
            (editForm.querySelector('#edit-description') as HTMLInputElement?)
                ?.value ??
            '';
        final quantityStr =
            (editForm.querySelector('#edit-quantity') as HTMLInputElement?)
                ?.value ??
            '';
        final unitVal =
            (editForm.querySelector('#edit-unit') as HTMLInputElement?)
                ?.value ??
            '';

        final editError =
            editForm.querySelector('#edit-error') as HTMLDivElement?;
        ...
        try {
          final updated = await expense_svc.updateExpense(
            ...
            UpdateExpenseRequest(
              ...
              quantity: quantityStr.isNotEmpty
                  ? num.tryParse(quantityStr)
                  : null,
      ...
      ((Event _) {
        () async {
          ...
          try {
            ...
            fileInput.value = '';
            final status = err is DioException
...
void _loadAttachmentList(Element container, String expenseId, bool isOwner) {
        ...
        for (final att in attachments) {
          ...
          if (att.contentType.startsWith('image/')) {
            final img = document.createElement('img') as HTMLImageElement
              ..src =
                  '/api/v1/expenses/$expenseId/attachments/${att.id}/content'
              ..alt = att.filename
...
HTMLFormElement _buildEditForm(Expense expense) {
  ...
  form.appendChild(
    _labeledInput('edit-category', 'Category', expense.category),
  );
  ...
  form.appendChild(
    _labeledInput('edit-description', 'Description', expense.description),
  );

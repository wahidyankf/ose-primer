(ns crud-be-cjpd.db.jdbc-expense-repo
  "JDBC implementation of ExpenseRepo backed by next.jdbc."
  (:require [clojure.string :as str]
            [next.jdbc :as jdbc]
            [next.jdbc.result-set :as rs]
            [crud-be-cjpd.db.protocols :as proto]
            [crud-be-cjpd.domain.expense :as expense-domain])
...
(defrecord JdbcExpenseRepo [ds]
  ...
  (update-expense! [this id {:keys [type amount currency description category unit quantity date]}]
    (let [now (now-str)]
      (jdbc/execute! ds
                     ["UPDATE expenses SET type = ?, amount = ?, currency = ?, description = ?, category = ?, unit = ?, quantity = ?, date = ?, updated_at = ?
                      ...
                      (when quantity (str quantity))
                      date now id])
      (proto/find-expense-by-id this id)))
...
(defn create
  "Create a JdbcExpenseRepo backed by the given datasource."
  [ds]
  (->JdbcExpenseRepo ds))

(ns step-definitions.steps
  ...
  (:require [clojure.string :as str]
            ...
            [crud-be-cjpd.db.protocols :as proto]
...
;; ============================================================
;; Test-support API (test-support/test-api.feature)
;; ============================================================

(Given "the test API is enabled via ENABLE_TEST_API environment variable" [state]
  ;; This feature's Background does not use "the API is running" (shared
  ;; across all crud-be-* language variants), so this step is what bootstraps
  ;; the scenario's repos/config — mirroring "the API is running" above.
       (if (:ds state)
         state
         (merge state (common/start-test-server!))))

(Given "users and expenses exist in the database" [state]
       (let [new-state    (-> state
                              (common/register-user! "resetdb-seed-user")
                              (common/login-user! "resetdb-seed-user"))
             token        (common/get-access-token new-state "resetdb-seed-user")
             expense-state (common/call-create-expense new-state token
                                                       (json/generate-string
                                                        {:amount      "10.00"
                                                         :currency    "USD"
                                                         :category    "food"
                                                         ...
                                                         :date        "2025-01-15"
                                                         :type        "expense"}))
             expense-id   (get-in expense-state [:last-body :id])
             data         (byte-array [0xFF 0xD8 0xFF 0xE0])]
         (common/call-upload-attachment expense-state token expense-id
                                        "seed.jpg" "image/jpeg" data)))

(Given "a user {string} exists" [state username]
       (common/register-user! state username))

(When "a POST request is sent to {string}" [state path]
      (cond
        (= "/api/v1/test/reset-db" path)
        (let [result (common/reset-test-db! state)]
          (assoc state
...
(When "a POST request is sent to {string} with body:" [state path table]
      (cond
        (= "/api/v1/test/promote-admin" path)
        (let [params   (into {} table)
              username (get params "username")
              result   (common/promote-admin-by-username! state username)]
          (assoc state
                 :last-response result
                 :last-body (:body result)))
...
(Then "the response status should be {int}" [state status-code]
      (is (= status-code (:status (:last-response state)))
          (str "Expected " status-code " but got " (:status (:last-response state))
               "\nBody: " (:last-body state)))
      state)

(Then "all user accounts should be deleted" [state]
      (let [cnt (proto/count-users (:user-repo state))]
        (is (zero? cnt) (str "Expected all user accounts to be deleted but found " cnt)))
      state)

(Then "all expenses should be deleted" [state]
      (let [cnt (common/count-all-expenses state)]
...
(Then "user {string} should have the {string} role" [state username expected-role]
      (let [user (proto/find-user-by-username (:user-repo state) username)]
        ...
        (is (= expected-role (:role user))
            (str "Expected user " username " to have role " expected-role
                 " but got " (:role user))))
      state)

...
Feature files in `specs/apps/crud/behavior/crud-be/gherkin/` are the source of truth:
...
```
specs/apps/crud/behavior/crud-be/gherkin/**/*.feature    ← source of truth (read-only)
        │
...
```bash
cd infra/dev/crud-be-golang-gin

...
const feature = await loadFeature(
  path.resolve(process.cwd(), "../../specs/apps/a-demo/fe/gherkin/token-management/tokens.feature"),
);
...
describeFeature(feature, ({ Scenario, Background }) => {
  ...
  Scenario("Session info displays the authenticated user's identity", ({ When, Then }) => {
    When("alice opens the session info panel", async () => {
      vi.mocked(tokensApi.getTokenClaims).mockResolvedValue(mockClaims);
      vi.mocked(tokensApi.getJwks).mockResolvedValue({
        keys: [{ kty: "oct", kid: "key-1", use: "sig", n: "mock-n", e: "AQAB" }],
      });
      const TokensPage = (await import("@/app/(dashboard)/tokens/page")).default;
      render(
      ...
      await waitFor(() => {
        expect(screen.getByTestId("token-subject")).toBeInTheDocument();
      });
    });

    Then("the panel should display alice's user ID", () => {
      expect(screen.getByTestId("token-subject").textContent).toBe("user-1");
    });
  });
  ...
  Scenario("Session info shows the token issuer", ({ When, Then }) => {
    When("alice opens the session info panel", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText("a-demo-fs-ts-nextjs")).toBeInTheDocument();
      });
    });

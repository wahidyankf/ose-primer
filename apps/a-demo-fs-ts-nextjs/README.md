...
```bash
# Development
nx dev a-demo-fs-ts-nextjs              # Start dev server on port 3401
nx build a-demo-fs-ts-nextjs            # Production build
nx run a-demo-fs-ts-nextjs:typecheck    # Type checking
nx run a-demo-fs-ts-nextjs:lint         # Linting (oxlint)

# Testing
nx run a-demo-fs-ts-nextjs:test:unit    # Run all unit tests (BE + FE BDD)
nx run a-demo-fs-ts-nextjs:test:quick   # Unit tests + coverage validation (75%+)

# Code generation
nx run a-demo-fs-ts-nextjs:codegen      # Generate types from OpenAPI spec
```
...
3. **E2E** (`test:e2e`): Full HTTP via `a-demo-be-e2e` and `a-demo-fe-e2e` Playwright suites.

## Related Documentation

- [OpenAPI Contract](../../specs/apps/a-demo/contracts/)
- [BE Gherkin Specs](../../specs/apps/a-demo/be/gherkin/)
- [FE Gherkin Specs](../../specs/apps/a-demo/fe/gherkin/)

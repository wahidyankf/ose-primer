...
let generateAccessToken (userId: Guid) (username: string) (email: string) (role: string) : string =
    ...
    let token =
        JwtSecurityToken(
            issuer = "crud-be-fsharp-giraffe",
            audience = "crud-be-fsharp-giraffe",
            claims = claims,
            notBefore = now,
...
let validateToken (token: string) : ClaimsPrincipal option =
    ...
    let validationParams =
        TokenValidationParameters(
            ...
            ValidateIssuer = true,
            ValidIssuer = "crud-be-fsharp-giraffe",
            ValidateAudience = true,
            ValidAudience = "crud-be-fsharp-giraffe",
...
let getJwks () =
    let key = getSigningKey ()
    let keyId = "crud-be-fsharp-giraffe-key-1"

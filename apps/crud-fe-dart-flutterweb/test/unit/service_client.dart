...
import 'package:crud_fe_dart_flutterweb/models/attachment.dart';
...
import 'package:crud_fe_dart_flutterweb/models/report.dart';
...
final _allowedExtensions = {'.jpg', '.jpeg', '.png', '.gif', '.pdf'};
...
class ServiceClient {
  ...
  Future<UserListResponse> listUsers({
    ...
    final slice = start >= total
        ? <_UserRecord>[]
        : all.skip(start).take(size).toList();
  ...
  Future<ExpenseSummary> getExpenseSummary({
    ...
    _requireAuth();
    final userId = _currentUserId!;
    var all = _expenses.values
        .where((e) => e.userId == userId && e.currency == currency)
        .toList();
    if (startDate != null) {
  ...
  Future<Attachment> uploadAttachment(
    ...
    if (!_allowedExtensions.contains(ext) ||
        !_allowedContentTypes.contains(ct)) {

...
void render(Element parent) {
  ...
  createForm.appendChild(
    buildFieldGroup('Amount', amountInput, 'amount-error'),
  );
  ...
  final currencyList = document.createElement('datalist')..id = 'currency-list';
  ...
  final currencyGroup = buildFieldGroup(
    'Currency',
    currencyInput,
    'currency-error',
  );
  ...
  createForm.appendChild(
    buildFieldGroup('Category', categoryInput, 'category-error'),
  );
  ...
  final descriptionGroup = buildFieldGroup(
    'Description',
    descriptionInput,
    'description-error',
  ...
  createForm.appendChild(
    buildFieldGroup('Quantity (optional)', quantityInput, 'quantity-error'),
  );
  ...
  newExpenseBtn.addEventListener(
    ...
    ((Event _) {
      final hidden =
          createFormWrapper.style.getPropertyValue('display') == 'none';
      ...
      if (result.content.isEmpty) {
        ...
        if (result.totalPages > 0) {
          ...
          final nextBtn = document.createElement('button') as HTMLButtonElement
            ...
            ..style.setProperty(
              'cursor',
              isLastPage ? 'not-allowed' : 'pointer',
            );
  ...
  createForm.addEventListener(
    ...
    ((Event e) {
      ...
      if (currencyVal.isEmpty) {
        ...
        setFieldError(
          'currency-error',
          'Unsupported currency. Use USD or IDR.',
        );
        currencyInput.setAttribute('aria-invalid', 'true');
        valid = false;
      }
      ...
      if (unitVal.isNotEmpty &&
          !_supportedUnits.contains(unitVal.toLowerCase())) {
        setFieldError(
          'unit-error',
          'Unsupported unit. Use a valid unit of measure.',
        );
      ...
      () async {
        try {
          await expense_svc.createExpense(
            CreateExpenseRequest(
              ...
              date: dateVal,
              type: typeVal,
              quantity: quantityVal.isNotEmpty
                  ? num.tryParse(quantityVal)
                  : null,

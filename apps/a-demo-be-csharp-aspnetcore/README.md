...
twin of `a-demo-be-golang-gin` (Go/Gin), `a-demo-be-elixir-phoenix` (Elixir Phoenix), `a-demo-be-fsharp-giraffe`
...
```bash
cd infra/dev/a-demo-be-csharp-aspnetcore
...
nx run a-demo-be-csharp-aspnetcore:test:integration
```
...
| ------------------ | ----------------------------------------------------- | --------------------------------------------------------- |
| `codegen`          | `nx run a-demo-be-csharp-aspnetcore:codegen`          | Generate contract types from OpenAPI spec                 |
| `build`            | `nx build a-demo-be-csharp-aspnetcore`                | Publish release artifact to `dist/` (depends on codegen)  |
| `dev`              | `nx dev a-demo-be-csharp-aspnetcore`                  | Hot-reload development server                             |
| `start`            | `nx start a-demo-be-csharp-aspnetcore`                | Run without hot-reload                                    |
| `test:quick`       | `nx run a-demo-be-csharp-aspnetcore:test:quick`       | All tests (SQLite) + Coverlet LCOV coverage ≥90% (cached) |
| `test:unit`        | `nx run a-demo-be-csharp-aspnetcore:test:unit`        | All tests (SQLite in-memory, no coverage report)          |
| `test:integration` | `nx run a-demo-be-csharp-aspnetcore:test:integration` | BDD scenarios against real PostgreSQL via docker-compose  |
| `lint`             | `nx run a-demo-be-csharp-aspnetcore:lint`             | Run Roslyn analyzers                                      |
| `typecheck`        | `nx run a-demo-be-csharp-aspnetcore:typecheck`        | Build with TreatWarningsAsErrors (depends on codegen)     |

...
Kotlin + Ktor REST API backend — a functional twin of `a-demo-be-golang-gin` (Go/Gin),
`a-demo-be-elixir-phoenix` (Elixir/Phoenix), and `a-demo-be-fsharp-giraffe` (F#/Giraffe) using Kotlin, Ktor, and Exposed.
...
| ------------------- | -------------------------------------------------------- | ------------------- |
| `PORT`              | `8201`                                                   | HTTP port           |
| `DATABASE_URL`      | `jdbc:postgresql://localhost:5432/a_demo_be_kotlin_ktor` | JDBC connection URL |
| `DATABASE_USER`     | `a_demo_be_kotlin_ktor`                                  | Database username   |
| `DATABASE_PASSWORD` | `a_demo_be_kotlin_ktor`                                  | Database password   |
...
```bash
# Start PostgreSQL
docker compose -f ../../infra/dev/a-demo-be-kotlin-ktor/docker-compose.yml up -d a-demo-be-db
...
```bash
nx build a-demo-be-kotlin-ktor              # Compile and package fat JAR (depends on codegen)
nx dev a-demo-be-kotlin-ktor               # Start development server
nx start a-demo-be-kotlin-ktor             # Start production JAR
nx run a-demo-be-kotlin-ktor:codegen       # Generate contract types from OpenAPI spec
nx run a-demo-be-kotlin-ktor:test:quick    # Unit tests + coverage gate (no lint)
nx run a-demo-be-kotlin-ktor:test:unit     # Unit tests only (Cucumber + JUnit)
nx run a-demo-be-kotlin-ktor:test:integration  # Integration tests (Docker + PostgreSQL)
nx lint a-demo-be-kotlin-ktor              # Run detekt linter
nx run a-demo-be-kotlin-ktor:typecheck     # Compile Kotlin sources (./gradlew compileKotlin, depends on codegen)
```

## API Endpoints

See [plan README](../../plans/done/2026-03-11__a-demo-be-kotlin-ktor/README.md) for the full API
surface.

## Test Architecture

Three-level testing strategy following the same pattern as `a-demo-be-golang-gin`:
...
### Level 3: E2E Tests (`a-demo-be-e2e`)

Playwright E2E tests in `apps/a-demo-be-e2e/` run against a real PostgreSQL database via
...
(`specs/apps/a-demo/be/gherkin/`). The `cucumber.glue` system property in each Gradle task controls
which step definitions are used:

- `testUnit`: `cucumber.glue=com.aademobektkt.unit.steps`
- `testIntegration`: `cucumber.glue=com.aademobektkt.integration.steps`

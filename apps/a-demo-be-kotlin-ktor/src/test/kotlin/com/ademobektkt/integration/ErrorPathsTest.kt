package com.ademobektkt.integration

import com.ademobektkt.auth.PasswordService
import com.ademobektkt.integration.steps.JsonHelper
import com.ademobektkt.integration.steps.ServiceDispatcher
import com.ademobektkt.integration.steps.TestDatabase
import com.ademobektkt.integration.steps.TestWorld
...
class ErrorPathsTest {
  ...
  fun setup() {
    ...
    aliceToken = JsonHelper.getString(loginBody, "accessToken") ?: error("No token in: $loginBody")
    ...
    adminToken =
      JsonHelper.getString(adminLoginBody, "accessToken") ?: error("No token in: $adminLoginBody")
  }
  ...
  fun `delete non-existent expense returns 404`() {
    val (status, _) = ServiceDispatcher.deleteExpense(aliceToken, UUID.randomUUID().toString())
    assertEquals(404, status)
  }
  ...
  fun `non-admin user gets 403 on admin force-password-reset endpoint`() {
    val (status, _) = ServiceDispatcher.forcePasswordReset(aliceToken, UUID.randomUUID().toString())
    assertEquals(403, status)
  }
  ...
  fun `admin force-password-reset non-existent user returns 404`() {
    val (status, _) = ServiceDispatcher.forcePasswordReset(adminToken, UUID.randomUUID().toString())
    assertEquals(404, status)
  }
  ...
  fun `get attachments for non-existent expense returns 404`() {
    val (status, _) = ServiceDispatcher.listAttachments(aliceToken, UUID.randomUUID().toString())
    assertEquals(404, status)
  }

  @Test
  fun `delete attachment with invalid attachment UUID returns 404`() {
    val (status, _) = ServiceDispatcher.deleteAttachment(aliceToken, aliceExpenseId, "not-a-uuid")
    assertEquals(404, status)
  }
  ...
  fun `delete attachment with invalid expense UUID returns 404`() {
    val (status, _) =
      ServiceDispatcher.deleteAttachment(aliceToken, "not-a-uuid", UUID.randomUUID().toString())
    assertEquals(404, status)
  }

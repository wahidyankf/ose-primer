      ...
      <div className={cardCn}>
        ...
        {claims && (
          <dl className="m-0">
            {[
              ...
              [
                "Roles",
                Array.isArray(claims["roles"]) ? (claims["roles"] as string[]).join(", ") : (claims["role"] ?? "—"),
              ],
            ].map(([label, value]) => (
              <div key={String(label)} className="mb-3 flex gap-4 border-b border-gray-100 pb-3">
                <dt className="min-w-[10rem] text-sm font-semibold text-gray-600">{String(label)}</dt>
                <dd
                  data-testid={String(label) === "Subject (User ID)" ? "token-subject" : undefined}
                  className="m-0 font-mono text-sm break-all"
                >
                  {String(value ?? "—")}
                </dd>
              </div>
        ...
        {claims && (
          <details className="mt-4">
            ...
            <pre className="mt-3 overflow-x-auto rounded bg-gray-50 p-4 text-xs">{JSON.stringify(claims, null, 2)}</pre>
          </details>
        )}
      </div>
                ...
                <li key={key.kid} className="mb-3 rounded-md border border-gray-200 bg-gray-50 p-4">
                  <dl className="m-0">
                    {[
                      ...
                      <div key={String(label)} className="mb-1.5 flex gap-4">
                        <dt className="min-w-[9rem] text-sm font-semibold text-gray-600">{String(label)}</dt>
                        <dd className="m-0 font-mono text-sm">{String(value)}</dd>
                      </div>
                    ))}
                  </dl>
                </li>

...
class IntegrationTestSupportSteps {
      ...
      path.endsWith("/test/promote-admin") -> {
        ...
        runBlocking {
          ...
          if (user == null) {
            ...
            transaction {
              UsersTable.update({ UsersTable.username eq username }) { it[role] = Role.ADMIN }
            }
            TestWorld.lastResponseStatus = 200
            TestWorld.lastResponseBody =
  ...
  // @covers specs/apps/crud/behavior/crud-be/gherkin/test-support/test-api.feature:Reset database
  ...
  // @covers specs/apps/crud/behavior/crud-be/gherkin/test-support/test-api.feature:Promote user to
  // admin role
  @And("user {string} should have the {string} role")
  fun userShouldHaveTheRole(username: String, expectedRole: String) {
    runBlocking {
      val user = TestWorld.userRepo.findByUsername(username) ?: error("User '$username' not found")
      assertEquals(

...
class AttachmentSteps {
  ...
  fun bobHasCreatedEntry(
    ...
    if (!TestWorld.accessTokens.containsKey("bob")) {
      for (pwd in passwords) {
        ...
        if (status == 200) {
          JsonHelper.getString(body, "accessToken")?.let { TestWorld.accessTokens["bob"] = it }
          break
        }
      }
    }
  ...
  // for an entry returns all uploaded files with metadata

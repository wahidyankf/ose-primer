...
/**
 * RED: Application.kt uses `?: "dev-jwt-secret-..."` so missing var falls back silently. GREEN will
 * replace with `error(...)` / `requireNotNull(...)` so absent var terminates startup.
 */
class ConfigSecurityTest {

  @Test
  fun `Application_kt has no soft default for jwt secret`() {
    val src = Files.readString(Path.of("src/main/kotlin/com/demobektkt/Application.kt"))
    assertFalse(

...
Kotlin + Ktor REST API backend — a functional twin of `crud-be-golang-gin` (Go/Gin),
`crud-be-elixir-phoenix` (Elixir/Phoenix), and `crud-be-fsharp-giraffe` (F#/Giraffe) using Kotlin, Ktor, and Exposed.
...
```bash
# Start PostgreSQL
docker compose -f ../../infra/dev/crud-be-kotlin-ktor/docker-compose.yml up -d crud-be-db
...
Three-level testing strategy following the same pattern as `crud-be-golang-gin`:
...
### Level 3: E2E Tests (`crud-be-e2e`)

Playwright E2E tests in `apps/crud-be-e2e/` run against a real PostgreSQL database via
...
(`specs/apps/crud/behavior/crud-be/gherkin/`). The `cucumber.glue` system property in each Gradle task controls

// Package server initializes and runs the HTTP server for the a-demo-be-golang-gin application.
package server
...
func Run() {
	...
	if cfg.DatabaseURL != "" {
		...
		db, err = gorm.Open(sqlite.Open("a-demo-be-golang-gin.db"), &gorm.Config{})
	}
	if err != nil {
	...
	log.Printf("Starting a-demo-be-golang-gin on %s", addr)

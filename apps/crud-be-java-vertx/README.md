...
| ------------------- | ----------------------------------------------------- | ------------------- |
| `PORT`              | `8201`                                                | HTTP port           |
| `DATABASE_URL`      | `jdbc:postgresql://localhost:5432/crud_be_java_vertx` | JDBC connection URL |
| `DATABASE_USER`     | `crud_be_java_vertx`                                  | Database username   |
| `DATABASE_PASSWORD` | `crud_be_java_vertx`                                  | Database password   |
...
```bash
# Start PostgreSQL
docker compose -f ../../infra/dev/crud-be-java-vertx/docker-compose.yml up -d crud-be-java-vertx-db
...
- **Cucumber BDD scenarios**: All Gherkin feature files from
  `specs/apps/crud/be/gherkin/` run against an in-process Vert.x HTTP server
...
Runs Playwright against any backend implementation. See `apps/crud-be-e2e/`.

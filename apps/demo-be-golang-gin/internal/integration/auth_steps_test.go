...
func (ctx *ScenarioCtx) aUserIsRegisteredAndDeactivated(username string) error {
	// Register (if not already registered) and login to get token, then deactivate.
	password := "Str0ng#Pass1"
	// Try to register; ignore conflict (already exists) errors.
	regErr := ctx.aUserIsRegisteredWithPassword(username, password)
	if regErr != nil {
		// Check if already exists — that's fine for this step.
		_ = regErr
	}
...
func (ctx *ScenarioCtx) theResponseBodyShouldContainValidationErrorFor(field string) error {
	body := parseBody(ctx.LastBody)

...
func (ctx *ScenarioCtx) alicesRefreshTokenHasExpired() error {
	// Inject an expired refresh token into the store.
...
func (ctx *ScenarioCtx) aliceSendsRefreshWithOriginalRefreshToken() error {
	// The original token was used in aliceHasUsedRefreshTokenToGetNewPair; now try to use it again.
	// We need to get the original token. Since we stored it in LastBody, let's extract it.
	var parsed map[string]interface{}
...
func (ctx *ScenarioCtx) theUserHasBeenDeactivated(username string) error {
	// Find user and deactivate directly via store.

...
| Gherkin specs     | **Must consume** shared specs from `specs/apps/crud/be/gherkin/`    |
...
All three levels consume the same shared Gherkin scenarios from [`specs/apps/crud/be/gherkin/`](../../../specs/apps/crud/be/gherkin/README.md). The difference is HOW the step definitions execute them:
...
specs because they use a separate spec directory (`specs/apps/crud/fe/gherkin/`).
...
| --------- | ---------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| 90%       | Demo-be backends, CLI apps (Go), Go libs, TS backend (`crud-be-ts-effect`)                                 | Core business logic with high mock isolation; all execution paths reachable in unit tests           |
| 80%       | Content platforms (`crud-fs-ts-nextjs`, `crud-fs-ts-nextjs`)                                               | Significant UI rendering code; some React rendering paths are hard to unit-test                     |
| 75%       | Fullstack (`crud-fs-ts-nextjs`)                                                                            | Mixed server+client code; API route handlers and React components share the codebase                |
| 70%       | FE apps (`crud-fe-ts-nextjs`, `crud-fe-ts-tanstack-start`, `crud-fe-dart-flutterweb`, `crud-fe-ts-nextjs`) | API/auth/query layers are fully mocked by design; threshold reflects intentional mocking boundaries |
...
- **Demo apps**: OpenAPI 3.1 spec at `specs/apps/crud/contracts/`; the `codegen` Nx target generates types and encoders/decoders into `generated-contracts/` (gitignored)
- **Content platforms and demo**: Use tRPC, which is typed at compile time without a separate contract file; demo also maintains an OpenAPI 3.1 spec at `specs/apps/crud/contracts/`
...
| `test-crud-be-*.yml`    | Yes  | No               | Yes           | Yes              | Yes      | Manual only   |
| `test-crud-fe-*.yml`    | Yes  | No               | Yes           | No               | Yes      | Manual only   |
...
- **TypeScript UI projects** (`crud-fe-ts-nextjs`, `crud-fe-ts-tanstack-start`,
  `crud-fs-ts-nextjs`, `crud-fe-ts-nextjs`, `crud-fs-ts-nextjs`, `crud-fs-ts-nextjs`, `libs/ts-ui`):
  `oxlint --jsx-a11y-plugin`
- **Dart UI projects** (`crud-fe-dart-flutterweb`): `dart analyze`
...
| ------------------------------ | ------------------------------- | ------------------------------------- | ------------------------------------ | ---------- | -------------------------------------- |
| Demo-be API backend            | All mocked + specs              | Real PostgreSQL, no HTTP + specs      | Playwright + specs                   | Yes        | `specs/apps/crud/be/gherkin/`          |
...
| Demo-fe frontend               | Vitest/Flutter mocks + specs    | N/A                                   | Playwright (via crud-fe-e2e) + specs | Yes        | `specs/apps/crud/fe/gherkin/`          |
...
- **Fullstack apps**: All three levels mandatory; consume Gherkin specs from both `specs/apps/crud/be/gherkin/` and `specs/apps/crud/fe/gherkin/` (the FS app spans both layers)
...
- **Demo-fe frontends**: Two-level testing (unit + E2E); no integration tier; all consume Gherkin specs from `specs/apps/crud/fe/gherkin/`; E2E via centralized `crud-fe-e2e` Playwright suite

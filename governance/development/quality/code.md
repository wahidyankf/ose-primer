...
## crud-fs-ts-nextjs Link Validation

Internal links in crud-fs-ts-nextjs content are validated
...
nx run crud-fs-ts-nextjs:test:quick

# Link check only (standalone)
nx run crud-fs-ts-nextjs:links:check
```
...
projects: `apps/crud-be-fsharp-giraffe-exph`, `libs/elixir-gherkin`, and `libs/elixir-cabbage`.
...
```bash
# From a project root (e.g. apps/crud-be-fsharp-giraffe-exph)
mix format

---
name: ayokoding-fs-by-example-quality-gate
...
outputs:
  ...
  - name: checker-report
    type: file
    pattern: generated-reports/ayokoding-fs-by-example__*__*__audit.md
    description: Final validation report from apps-ayokoding-fs-by-example-checker (4-part format with UUID chain)
  - name: fixer-report
    type: file
    pattern: generated-reports/ayokoding-fs-by-example__*__*__fix.md
    description: Final fixes report from apps-ayokoding-fs-by-example-fixer (4-part format with UUID chain)
...
- Before publishing by-example content to ayokoding-fs
...
**Preferred Mode**: Agent Delegation — invoke `apps-ayokoding-fs-by-example-checker` and
`apps-ayokoding-fs-by-example-fixer` via the Agent tool with `subagent_type`
...
```
User: "Run ayokoding-fs by-example quality gate workflow for golang/tutorials/by-example/"
```

The AI will:

1. Invoke `apps-ayokoding-fs-by-example-checker` via the Agent tool (validates tutorial, writes audit)
2. User reviews audit report and decides on fixes (manual decision point)
3. Invoke `apps-ayokoding-fs-by-example-fixer` via the Agent tool (reads audit, applies fixes, writes fix report)
...
```
User: "Run ayokoding-fs by-example quality gate workflow for golang/tutorials/by-example/ in manual mode"
```
...
graph TB
    ...
    A -->|apps-ayokoding-fs-by-example-maker or manual| B
    B -->|apps-ayokoding-fs-by-example-checker| C
    C -->|Issues found| D
    D -->|apps-ayokoding-fs-by-example-fixer| B
...
**Agent**: `apps-ayokoding-fs-by-example-checker`

**Execution**:

```bash
# Invoke via Task tool
subagent_type: apps-ayokoding-fs-by-example-checker
prompt: "Validate apps/ayokoding-fs/content/en/learn/software-engineering/programming-language/golang/tutorials/by-example/ for compliance with by-example standards"
```
...
- Audit report: `generated-reports/ayokoding-fs-by-example__{uuid-chain}__{timestamp}__audit.md`
...
**Agent**: `apps-ayokoding-fs-by-example-fixer`

**Execution**:

```bash
# Invoke via Task tool with audit report and mode parameter
subagent_type: apps-ayokoding-fs-by-example-fixer
prompt: "Apply fixes from generated-reports/ayokoding-fs-by-example__a1b2c3__2025-12-25--14-30__audit.md with mode={input.mode}"
```
...
- Fix report: `generated-reports/ayokoding-fs-by-example__{uuid-chain}__{timestamp}__fix.md` (uses same UUID chain as source audit)
...
```bash
apps-ayokoding-fs-by-example-checker validates golang by-example
```
...
```bash
apps-ayokoding-fs-by-example-fixer applies fixes from audit
```
...
```bash
apps-ayokoding-fs-by-example-checker re-validates
```
...
```bash
apps-ayokoding-fs-by-example-checker validates elixir by-example
```
...
```bash
apps-ayokoding-fs-by-example-fixer applies fixes
```
...
```bash
apps-ayokoding-fs-by-example-checker validates java by-example
```
...
```
User: "Run ayokoding-fs by-example quality gate workflow for golang/tutorials/by-example/ in normal mode"
```
...
```
User: "Run ayokoding-fs by-example quality gate workflow for elixir/tutorials/by-example/ in strict mode"
```
...
```
User: "Run ayokoding-fs by-example quality gate workflow for java/tutorials/by-example/ in ocd mode with max-iterations=10"
```
...
2. **Validate**: Invoke `apps-ayokoding-fs-by-example-checker` via Agent tool
3. **Review**: User reads audit report from generated-reports/
4. **Fix**: Invoke `apps-ayokoding-fs-by-example-fixer` via Agent tool
...
```
User: "Run ayokoding-fs by-example quality gate workflow for golang/tutorials/by-example/ in strict mode with max-iterations=10"
```
...
- **ayokoding-fs-by-example-quality-gate** (this workflow): Specialized for by-example tutorials
- **ayokoding-fs workflow**: Hugo content validation
...
- **[`apps-ayokoding-fs-by-example-checker` agent](../../../.claude/agents/apps-ayokoding-fs-by-example-checker.md)**: Validation agent
- **[`apps-ayokoding-fs-by-example-fixer` agent](../../../.claude/agents/apps-ayokoding-fs-by-example-fixer.md)**: Fixing agent
- **[`apps-ayokoding-fs-by-example-maker` agent](../../../.claude/agents/apps-ayokoding-fs-by-example-maker.md)**: Content creation agent

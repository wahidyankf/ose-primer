---
name: ayokoding-fs-general-quality-gate
goal: Validate all ayokoding-fs content quality, apply fixes iteratively until zero findings
...
inputs:
  - name: scope
    type: string
    description: Content to validate (e.g., "all", "ayokoding-fs/content/en/", "specific-file.md")
    required: false
    default: all
  - name: min-iterations
    type: number
...
outputs:
  ...
  - name: content-report
    type: file
    pattern: generated-reports/ayokoding-fs-general__*__audit.md
    description: Final content validation report
  - name: facts-report
    type: file
    pattern: generated-reports/ayokoding-fs-facts__*__audit.md
    description: Final facts validation report
  - name: links-report
    type: file
    pattern: generated-reports/ayokoding-fs-link__*__audit.md
    description: Final links validation report
---

# AyoKoding Content General Quality Gate Workflow

**Purpose**: Comprehensively validate all ayokoding-fs content (factual accuracy, links), apply fixes iteratively until all issues are resolved.

**When to use**:

- After creating or updating ayokoding-fs content
- Before deploying ayokoding-fs to production
- Periodically to ensure content quality and accuracy
- After bulk content changes or restructuring

## Execution Mode

**Preferred Mode**: Agent Delegation — invoke `apps-ayokoding-fs-general-checker`,
`apps-ayokoding-fs-facts-checker`,
`apps-ayokoding-fs-link-checker`, `apps-ayokoding-fs-general-fixer`,
and `apps-ayokoding-fs-facts-fixer`
...
```
User: "Run ayokoding-fs general quality gate workflow for ayokoding-fs/content/en/"
```
...
```
User: "Run ayokoding-fs general quality gate workflow for ayokoding-fs/content/en/ in manual mode"
```
...
**Agent**: `apps-ayokoding-fs-general-fixer`
...
**Agent**: `apps-ayokoding-fs-facts-fixer`
...
```
User: "Run ayokoding-fs general quality gate workflow"
```

The AI will invoke specialized agents via the Agent tool:

- Validate all ayokoding-fs content in parallel (`apps-ayokoding-fs-general-checker`, `apps-ayokoding-fs-facts-checker`, `apps-ayokoding-fs-link-checker` subagents)
- Fix all findings (`apps-ayokoding-fs-general-fixer`, `apps-ayokoding-fs-facts-fixer` subagents)
...
```
User: "Run ayokoding-fs general quality gate workflow for ayokoding-fs/content/en/programming/"
```
...
```
User: "Run ayokoding-fs general quality gate workflow with min-iterations=2 and max-iterations=10"
```
...
### Content Validation (apps-ayokoding-fs-general-checker)

- Content quality principles
- Bilingual consistency

### Facts Validation (apps-ayokoding-fs-facts-checker)
...
### Links Validation (apps-ayokoding-fs-link-checker)
...
- Deployment workflows (validate before deploying ayokoding-fs)
...
This workflow ensures comprehensive ayokoding-fs content quality through multi-dimensional validation and iterative fixing.

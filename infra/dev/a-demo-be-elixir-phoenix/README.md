...
Local development environment for `a-demo-be-elixir-phoenix`, the Elixir/Phoenix
alternative backend for the Demo Backend platform. Runs on the same port (8201) as the
Go/Gin backend (`a-demo-be-golang-gin`) — the two stacks are mutually
...
| Service                     | Port |
| --------------------------- | ---- |
| a-demo-be-elixir-phoenix-db | 5432 |
| a-demo-be-elixir-phoenix    | 8201 |

## Quick Start

```bash
# From workspace root
cd infra/dev/a-demo-be-elixir-phoenix
...
The `a-demo-be-elixir-phoenix` container automatically runs `mix ecto.migrate`
...
| ------------------- | ------------------------------------------ | -------------------------- |
| `POSTGRES_USER`     | `a_demo_be_elixir_phoenix`                 | PostgreSQL username        |
| `POSTGRES_PASSWORD` | `a_demo_be_elixir_phoenix`                 | PostgreSQL password        |
...
names (`a_demo_be_elixir_phoenix` for this backend, `demo_be` for golang-gin/java-springboot, etc.) so they could share a PostgreSQL
...
`a-demo-be-elixir-phoenix` declares `elixir-gherkin` and `elixir-cabbage` as local

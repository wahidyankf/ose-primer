...
Local development environment for `crud-be-java-springboot`, the Java/Spring Boot
...
| ------------------------------------ | ------------------------------------------ | -------------------------------- |
| `POSTGRES_USER`                      | `demo`                                     | PostgreSQL username              |
| `POSTGRES_PASSWORD`                  | `demo`                                     | PostgreSQL password              |
| `CRUD_BE_JAVA_SPRINGBOOT_JWT_SECRET` | `change-me-in-dev-only-not-for-production` | JWT signing secret (HMAC-SHA256) |
...
docker compose -f docker-compose.yml -f docker-compose.ci.yml up --build -d crud-be-java-springboot
...
BASE_URL=http://localhost:3301 BACKEND_URL=http://localhost:8201 npx nx run crud-fe-e2e:test:e2e

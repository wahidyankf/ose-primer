...
Local development environment for `a-demo-be-fsharp-giraffe`, the F#/Giraffe
alternative backend for the Demo Backend platform. Runs on the same port (8201) as the
Go/Gin backend (`a-demo-be-golang-gin`) and the Elixir/Phoenix backend (`a-demo-be-elixir-phoenix`) — the
...
| Service                     | Port |
| --------------------------- | ---- |
| a-demo-be-fsharp-giraffe-db | 5432 |
| a-demo-be-fsharp-giraffe    | 8201 |

## Quick Start

```bash
# From workspace root
cd infra/dev/a-demo-be-fsharp-giraffe
...
| ------------------- | ------------------------------------------ | -------------------------------- |
| `POSTGRES_USER`     | `a_demo_be_fsharp_giraffe`                 | PostgreSQL username              |
| `POSTGRES_PASSWORD` | `a_demo_be_fsharp_giraffe`                 | PostgreSQL password              |

...
description: Authoritative demo F# functional programming standards — computation expressions, monadic composition, applicative validation
...
This document defines **authoritative functional programming standards** for F# development in demo. F# is a functional-first language — these patterns are idiomatic, not optional. OOP patterns (mutable classes, inheritance hierarchies) are explicitly avoided in domain code.

**Target Audience**: demo F# developers applying FP patterns in domain modeling and service logic

...
description: Authoritative demo F# type safety standards — making illegal states unrepresentable, units of measure, single-case DUs, generics
...
This document defines **authoritative type safety standards** for F# development in demo. F# offers some of the strongest type-level safety in the .NET ecosystem. The guiding principle: **make illegal states unrepresentable** — encode domain invariants in the type system so the compiler prevents mistakes before tests even run.

**Target Audience**: demo F# developers working on domain modeling, financial calculation, and data integration

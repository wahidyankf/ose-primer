...
description: Authoritative demo F# error handling standards — railway-oriented programming, Result type, Option, FsToolkit
...
This document defines **authoritative error handling standards** for F# development in demo. F# uses railway-oriented programming (ROP) with `Result<'T, 'TError>` for domain errors and `Option<'T>` for nullable values. Bare exceptions are reserved for infrastructure failures, not domain logic.

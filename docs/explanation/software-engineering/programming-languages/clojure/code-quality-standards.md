...
description: Authoritative demo Clojure code quality standards (clj-kondo, cljfmt, Eastwood)
...
This document defines the mandatory code quality toolchain for Clojure development in demo. All tools run automatically in pre-commit hooks and CI/CD — manual code quality checks are insufficient.

**Target Audience**: demo Clojure developers, DevOps engineers, automated quality tools
...
### Custom Lint Rules for demo

```clojure
;; CORRECT: Extended config for demo conventions
{:linters
 {:unused-namespace {:level :error}
  :unused-referred-var {:level :error}
  :unused-binding {:level :warning}

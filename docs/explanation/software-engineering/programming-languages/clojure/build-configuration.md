...
description: Authoritative demo Clojure build configuration standards (deps.edn, tools.build, Leiningen, Babashka)
...
This document defines the build configuration standards for Clojure projects in demo. `deps.edn` + `tools.deps` is the modern preferred approach; Leiningen remains supported for legacy projects.

**Target Audience**: demo Clojure developers, DevOps engineers
...
```clojure
;; CORRECT: Full deps.edn structure for demo Clojure project
{:paths ["src" "resources"]

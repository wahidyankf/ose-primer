...
description: Authoritative demo Clojure error handling standards (ex-info, structured errors, try/catch, Result maps)
...
This document defines authoritative error handling standards for Clojure development in demo. Clojure's `ex-info` with structured data maps provides richer error context than plain Java exceptions.

**Target Audience**: demo Clojure developers, technical reviewers
...
**MUST** use `ex-info` for all application-level exceptions in demo.

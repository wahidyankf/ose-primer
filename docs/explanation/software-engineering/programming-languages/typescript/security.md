...
**Layer 1 — Controller** (green): Extract parameters, validate structure.

```mermaid
graph LR
    CLIENT[Client Request]
    CONTROLLER[Controller Layer]
    WHITELIST{Valid structure?}
    SANITIZE[String Sanitization]
    REJECT_PARAM[Reject 400<br/>Invalid parameters]

    CLIENT --> CONTROLLER --> WHITELIST
...
graph LR
    ...
    REJECT_BIZ[Reject 400<br/>Validation errors]
    DB_CONSTRAINTS[DB Constraints<br/>Pass]
    REJECT_DB[Reject<br/>DB constraint]
    SUCCESS[Success<br/>Donation created]

    SANITIZE --> ZOD --> VALIDATE_BIZ
    VALIDATE_BIZ -->|Invalid| REJECT_BIZ
    VALIDATE_BIZ -->|Valid| DB_CONSTRAINTS

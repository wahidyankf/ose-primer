...
description: Authoritative demo Rust memory management standards (ownership, borrowing, lifetimes, smart pointers)
...
This document defines **authoritative memory management standards** for Rust development in the demo. Rust's ownership system is its most distinctive feature — it provides memory safety without a garbage collector, with zero runtime overhead.

**Target Audience**: demo Rust developers, especially those transitioning from GC languages

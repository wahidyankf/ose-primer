...
description: Authoritative demo Rust error handling standards (Result, Option, thiserror, anyhow)
...
This document defines **authoritative error handling standards** for Rust development in the demo. Rust's error handling model — using `Result<T, E>` and `Option<T>` — is a core language feature. These standards ensure consistent, informative, and maintainable error handling across all Rust projects.

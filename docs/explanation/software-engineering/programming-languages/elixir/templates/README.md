# Elixir Documentation Templates

This directory contains reusable code templates and patterns for Elixir development.

## Purpose

Templates provide standardized starting points for common Elixir patterns,
reducing repetition and ensuring consistency across documentation and code examples.

## Template Naming Convention

Templates use plain kebab-case filenames (e.g., `genserver-module.md`). The containing directory (`programming-languages/elixir/templates/`) encodes the category.

## Templates

This directory is currently a placeholder. Templates will be added as common patterns are identified.

Suggested starting template: `genserver-module.md`

## Usage

Templates in this directory can be used as:

- Starting points for new implementations
- Reference patterns for code review
- Educational examples in documentation

## Related Documentation

- [Elixir Programming Language Overview](../README.md) - Language documentation index
- [File Naming Convention](../../../../../../governance/conventions/structure/file-naming.md) - Naming patterns

---

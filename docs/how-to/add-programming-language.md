...
description: "Step-by-step guide for adding programming languages to crud-fs-ts-nextjs following the Programming Language Content Standard"
category: how-to
tags:
...
**Step-by-step guide for adding a new programming language to crud-fs-ts-nextjs following the Programming Language Content Standard.**

This guide walks you through the complete process of adding a new programming language (e.g., Kotlin, TypeScript, Rust, Clojure) to crud-fs-ts-nextjs. Follow these steps to ensure your content meets quality standards and integrates seamlessly with existing content.
...
- [ ] **AI agents available**:
  ...
  - `crud-fs-ts-nextjs-general-checker` (general quality validation)
  - `crud-fs-ts-nextjs-by-example-checker` (by-example tutorial validation)
  - `crud-fs-ts-nextjs-facts-checker` (factual verification)
  - `crud-fs-ts-nextjs-link-checker` (link validation)
...
**Why:** Ensure the language addition provides value to crud-fs-ts-nextjs users.
...
1. Navigate to crud-fs-ts-nextjs content directory:
...
1. **Use crud-fs-ts-nextjs-general-maker agent**:

   ```
   Spawn crud-fs-ts-nextjs-general-maker agent:
...
3. **Use crud-fs-ts-nextjs-general-maker agent**:
...
2. **Use crud-fs-ts-nextjs-general-maker agent**:
...
4. **Use crud-fs-ts-nextjs-general-maker agent**:
...
1. **Run crud-fs-ts-nextjs-general-checker**:
...
1. **Run crud-fs-ts-nextjs-facts-checker**:

   ```
   Spawn crud-fs-ts-nextjs-facts-checker agent:
...
1. **Run crud-fs-ts-nextjs-link-checker**:

   ```
   Spawn crud-fs-ts-nextjs-link-checker agent:
...
**Why:** Make content available to crud-fs-ts-nextjs users.

**Actions:**

1. **Verify on main branch**: Ensure all changes are committed to `main`

2. **Run crud-fs-ts-nextjs-deployer**:

   ```
   Spawn crud-fs-ts-nextjs-deployer agent:
   "Deploy crud-fs-ts-nextjs to production (example.com)

   This will sync prod-crud-fs-ts-nextjs branch with main and trigger Vercel deployment."
   ```
...
3. Deploy via crud-fs-ts-nextjs-deployer
...
**Issue:** crud-fs-ts-nextjs-facts-checker reports ❌ Error or 📅 Outdated
...
- [ ] crud-fs-ts-nextjs-general-checker audit clean
- [ ] crud-fs-ts-nextjs-facts-checker audit clean (✅ Verified)
- [ ] crud-fs-ts-nextjs-link-checker validation passed
...
- [ ] Deployed to production via crud-fs-ts-nextjs-deployer
...
**Congratulations!** You've successfully added a new programming language to crud-fs-ts-nextjs following the Programming Language Content Standard. Your content is now part of a consistent, high-quality educational platform.

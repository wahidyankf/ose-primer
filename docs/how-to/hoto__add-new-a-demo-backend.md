...
**`codegen` target** must depend on `a-demo-contracts:bundle`:
...
services:
  postgres:
    ...
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U a_demo_be_{abbrev}"]
      interval: 2s
      timeout: 5s
      retries: 10
  ...
  test-runner:
    ...
    environment:
      DATABASE_URL: postgresql://a_demo_be_{abbrev}:a_demo_be_{abbrev}@postgres:5432/a_demo_be_{abbrev}
    volumes:
      - ../../specs:/specs:ro
```
...
**CI test workflow** — Create `.github/workflows/test-a-demo-be-{lang}-{framework}.yml` for

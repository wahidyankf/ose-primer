...
nx run a-demo-be-golang-gin:test:unit
...
```bash
cd apps/a-demo-be-{lang}-{framework}
docker compose -f docker-compose.integration.yml down -v
docker compose -f docker-compose.integration.yml up --abort-on-container-exit --build
```
...
### Backend E2E (`a-demo-be-e2e`)

Tests live in `apps/a-demo-be-e2e/` and test any of the 11 backends.
...
### Frontend E2E (`a-demo-fe-e2e`)

Tests live in `apps/a-demo-fe-e2e/` and test any of the 3 frontends.
...
nx run a-demo-fe-ts-nextjs:dev
...
| E2E (BE)    | `nx run a-demo-be-e2e:test:e2e`     | Running backend | No     |
| E2E (FE)    | `nx run a-demo-fe-e2e:test:e2e`     | Running stack   | No     |
...
- [Frontend Gherkin Specs](../../specs/apps/a-demo/fe/gherkin/README.md)

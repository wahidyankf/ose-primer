...
| **`repo-config.yml` schema parity**                                                           | `rhino-cli repo-config validate` — strict-deserialize schema check (deny-unknown-fields + required/enum checks), wired at pre-commit (file-scoped fast path) and pre-push/PR (defense-in-depth)                  | Converts the "identical key set across `repo-config.yml`" boundary from prose review into an enforced, automated gate.                                                                                                                                                                                                |
...
carve-outs**. `apps/rhino-cli`'s `src/`, `Cargo.toml`, `Cargo.lock`, `project.json`, and `LICENSE` are
byte-identical across `ose-public`, `ose-primer`, and `ose-infra`. The canonical source carries the
...
(`rhino-cli repo-config validate`, run at pre-commit and again at pre-push/PR in every repo) enforces

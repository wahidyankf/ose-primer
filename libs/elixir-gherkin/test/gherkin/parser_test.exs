defmodule Gherkin.ParserTest do
  ...
  # ---- OSE fork coverage additions -----------------------------------------------
  # These tests exercise alternative Gherkin keywords and rule-level constructs that
  # the upstream test suite did not cover.  They keep the diff from upstream minimal
  # while satisfying the ≥ 90 % line-coverage gate enforced by project.json.

  @feature_with_example_keyword """
  Feature: Example keyword alias
    The word Example is a valid synonym for Scenario at the feature level.
  ...
  @feature_with_scenario_template """
  Feature: Scenario Template keyword alias
  ...
  @feature_with_rule_and_scenario_outline """
  Feature: Rules can contain Scenario Outlines

    Rule: Outline inside a rule
      Scenario Outline: eating
        Given there are <start> cucumbers
        When I eat <eat> cucumbers
        Then I should have <left> cucumbers
  ...
  @feature_with_string_valued_tag """
  @label production
  Feature: String-valued tag
    Tags can carry a string value that is not a number.

    Scenario: Tagged scenario
      Given something
  """

  test "Example: is accepted as a synonym for Scenario: at the feature level" do
    assert %{scenarios: [%{name: "A basic example"}]} =
             parse_feature(@feature_with_example_keyword)
  end

  test "Scenario Template: is accepted as a synonym for Scenario Outline:" do
    %{scenarios: [outline]} = parse_feature(@feature_with_scenario_template)
    assert outline.name == "eating"
    assert [%{start: "12", eat: "5", left: "7"}] = outline.examples
  end

  test "Scenario Outline: is accepted inside a Rule" do
    assert %{rules: [%Rule{scenarios: [outline]}]} =
             parse_feature(@feature_with_rule_and_scenario_outline)

    assert outline.name == "eating"
    assert [%{start: "12", eat: "5", left: "7"}] = outline.examples
  end

  test "a tag with a non-numeric string value is parsed correctly" do
    assert %{tags: [{:label, "production"}]} =
             parse_feature(@feature_with_string_valued_tag)
  end
end

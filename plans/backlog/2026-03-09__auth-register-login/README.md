...
- PostgreSQL `users` table managed by Flyway migrations
- Spring Data JPA + PostgreSQL `users` table managed by Flyway migrations
...
- Keep integration tests in-process with no external dependencies (H2 + Flyway)
...
1. `feat(organiclever-be): add DB dependencies and Flyway migration`

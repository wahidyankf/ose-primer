...
Rewrite `apps/oseplatform-fs` from Hugo (PaperMod theme) to Next.js 16, following the same architectural patterns established by `apps/ayokoding-fs`. The current Hugo site is a small marketing/landing site with approximately 6 pages (landing, about, 4 update posts). The new Next.js version will provide a modern, extensible platform for future growth while maintaining the same content, SEO characteristics, and deployment workflow.

This plan follows the proven approach from the [ayokoding-fs v2 rewrite](../../done/2026-03-23__ayokoding-web-v2/README.md) and [v1-to-v2 migration](../../done/2026-03-24__ayokoding-web-v1-to-v2-migration/README.md), adapted for the significantly simpler scope of oseplatform-fs.
...
3. **Inconsistent patterns** -- Every other web app in the monorepo uses Next.js 16; Hugo is the sole outlier after the ayokoding-fs migration
...
| **Dev port**       | 3000                                                                     |
| **Deployment**     | Vercel via `prod-oseplatform-fs` branch, 2x daily cron                   |
...
| **Dev port**   | 3100                                                                 |
| **Deployment** | Vercel via `prod-oseplatform-fs` branch (unchanged)                  |
...
- New Next.js 16 app at `apps/oseplatform-fs` (in-place replacement)
- Content layer with markdown parsing (reuse ayokoding-fs patterns)
...
- E2E test apps (dedicated `oseplatform-fs-be-e2e`/`oseplatform-fs-fe-e2e` -- can be added later)
...
| **API**       | tRPC                            | Type-safe, consistent with ayokoding-fs, supports future features |
| **Search**    | FlexSearch                      | Consistent with ayokoding-fs; lightweight for small content set   |
| **i18n**      | None (single locale)            | Site is English-only; no Indonesian content exists                |
| **Port**      | 3100                            | Distinct from Hugo (3000), close to ayokoding-fs (3101)           |
| **Coverage**  | 80%                             | Matches ayokoding-fs threshold                                    |
| **Dark mode** | Enabled (default: light)        | Content-first site per conventions; trivial with next-themes      |
| **Archival**  | `archived/oseplatform-fs-hugo/` | Preserves git history; same pattern as ayokoding-fs v1            |

## Differences from ayokoding-fs

| Aspect                | ayokoding-fs                                           | oseplatform-fs        |
| --------------------- | ------------------------------------------------------ | --------------------- |
...
- Current app: [`apps/oseplatform-fs/`](../../../apps/oseplatform-fs/)
- Reference app: [`apps/ayokoding-fs/`](../../../apps/ayokoding-fs/)
- CLI tool: [`apps/oseplatform-cli/`](../../../apps/oseplatform-cli/)
- Deployer agent: [`.claude/agents/apps-oseplatform-fs-deployer.md`](../../../.claude/agents/apps-oseplatform-fs-deployer.md)
- Content skill: [`.claude/skills/apps-oseplatform-fs-developing-content/`](../../../.claude/skills/apps-oseplatform-fs-developing-content/)

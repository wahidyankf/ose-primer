...
| ---- | -------------------------------------------------------------------- | ------------------------------------------ | ------------------------------------------------------------- |
| A    | `repo-governance/` → `repo-governance/`                              | All text files (excl. `.opencode/agents/`) | Path tokens with trailing slash                               |
| B    | `"governance"` → `"repo-governance"`                                 | `*.go` only                                | Bare quoted directory-name string literals + test fixtures    |
| C    | `repo-governance vendor-audit` → `repo-repo-governance vendor-audit` | All text files                             | CLI verb (space-separated, no slash)                          |
...
- `grep -qE '^repo-governance/workflows/'` → `grep -qE '^repo-governance/workflows/'`
- `grep -qE '^(repo-governance/.*\.md|...'` → `grep -qE '^(repo-governance/.*\.md|...'`

Key files caught by Pass A:

- All `.claude/agents/*.md` (52 files) — relative links into `repo-governance/conventions/`,
  `repo-governance/development/`, etc.
- All `.claude/skills/*/SKILL.md` (34 skill packages) — layer-to-path mappings,
  cross-references to governance conventions
- `repo-governance/conventions/structure/governance-vendor-independence.md` — path examples
- `repo-governance/workflows/` — all workflow files with self-referential paths
- `apps/rhino-cli/project.json` — target inputs `{workspaceRoot}/repo-governance/**/*.md`
...
| File                                                                      | Content                                         |
| ------------------------------------------------------------------------- | ----------------------------------------------- |
| `apps/rhino-cli/project.json`                                             | `repo-governance vendor-audit repo-governance/` |
| `apps/rhino-cli/scripts/validate-cross-vendor-parity.sh`                  | `repo-governance vendor-audit ...`              |
| `repo-governance/conventions/structure/governance-vendor-independence.md` | CLI usage examples                              |
| `specs/apps/rhino/cli/gherkin/repo-governance-vendor-audit.feature`       | Gherkin step text                               |
...
| `repo-governance/conventions/structure/governance-vendor-independence.md` | Nx target reference                                          |
| `repo-governance/conventions/structure/README.md`                         | Feature file entry reference                                 |
| `specs/apps/rhino/cli/gherkin/repo-governance-vendor-audit.feature`       | `@repo-governance-vendor-audit` tag                          |
...
**Note**: The `.feature` file is named `repo-governance-vendor-audit.feature` — it also needs a `git mv`
...
### Files in `internal/repo-governance/`
...
| `specs/apps/rhino/cli/gherkin/repo-governance-vendor-audit.feature` | `specs/apps/rhino/cli/gherkin/repo-governance-vendor-audit.feature` |
...
| `specs/apps/rhino/cli/gherkin/repo-governance-vendor-audit.feature`  | C, D + git mv | Wrong CLI verb in step text; tag mismatch               |
...
Parent repo has zero occurrences of `ose-primer/repo-governance/` — verified at plan creation time.
...
- A missed `repo-governance/` path token shows up as a broken reference at runtime or test time
...
git mv specs/apps/rhino/cli/gherkin/repo-governance-vendor-audit.feature \
       specs/apps/rhino/cli/gherkin/repo-governance-vendor-audit.feature
...
find . -not -path './.git/*' -not -path './node_modules/*' \
  ...
  | xargs grep -l 'repo-repo-governance vendor-audit' \
  | xargs sed -i '' 's|repo-repo-governance vendor-audit|repo-governance vendor-audit|g'

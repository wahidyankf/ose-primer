# Requirements: a-demo-be-java-vertx
...
- **Integration tests** (`test:integration`): Vert.x Test + in-memory repository
  implementations (`ConcurrentHashMap`-based) — fully in-process, no external services, safe
  to cache. Matches the monorepo convention established by `a-demo-be-java-springboot` (MockMvc +
  InMemoryDataStore) and `a-demo-be-elixir-phoenix` (InMemoryStore contexts).
...
- Coverage ≥ 90% (enforced by `rhino-cli test-coverage validate` against JaCoCo XML output)
...
- `rhino-cli java validate-annotations` validates `@NullMarked` presence on all packages
- NullAway (Error Prone plugin) enforces null safety at compile time
...
- Integration tests use in-memory mock repositories — no Testcontainers, no external services
- `test:integration` target is cacheable (fully deterministic, in-process only)

# Requirements: demo-be-java-vertx
...
- **Integration tests** (`test:integration`): Vert.x Test + in-memory repository
  ...
  to cache. Matches the monorepo convention established by `demo-be-java-springboot` (MockMvc +
  InMemoryDataStore) and `demo-be-elixir-phoenix` (InMemoryStore contexts).

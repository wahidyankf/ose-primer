# Requirements: a-demo-be-kotlin-ktor

All acceptance criteria are expressed in Gherkin and directly correspond to the scenarios in
`specs/apps/a-demo/be/gherkin/`. Acceptance is verified at two layers:

- **Integration tests** (`test:integration`): Ktor `testApplication {}` in-process HTTP testing
  with Exposed + SQLite in-memory database — fully in-process, no external services, safe to
  cache. Matches the monorepo convention established by `a-demo-be-java-springboot` (MockMvc +
  InMemoryDataStore), `a-demo-be-elixir-phoenix` (InMemoryStore), and `a-demo-be-fsharp-giraffe` (TestServer + SQLite).
- **E2E tests** (`test:e2e`): Playwright via `a-demo-be-e2e` against the live Dockerised stack at
  port 8201 — exercises the full stack including real PostgreSQL.

---

## Feature: Health Check

_Source: `specs/apps/a-demo/be/gherkin/health/health-check.feature`_ (2 scenarios)
...
_Source: `specs/apps/a-demo/be/gherkin/authentication/password-login.feature`_ (5 scenarios)
...
_Source: `specs/apps/a-demo/be/gherkin/authentication/token-lifecycle.feature`_ (7 scenarios)
...
_Source: `specs/apps/a-demo/be/gherkin/user-lifecycle/registration.feature`_ (6 scenarios)
...
_Source: `specs/apps/a-demo/be/gherkin/user-lifecycle/user-account.feature`_ (6 scenarios)
...
_Source: `specs/apps/a-demo/be/gherkin/security/security.feature`_ (5 scenarios)
...
_Source: `specs/apps/a-demo/be/gherkin/token-management/tokens.feature`_ (6 scenarios)
...
_Source: `specs/apps/a-demo/be/gherkin/admin/admin.feature`_ (6 scenarios)
...
_Source: `specs/apps/a-demo/be/gherkin/expenses/expense-management.feature`_ (7 scenarios)
...
_Source: `specs/apps/a-demo/be/gherkin/expenses/currency-handling.feature`_ (6 scenarios)
...
_Source: `specs/apps/a-demo/be/gherkin/expenses/unit-handling.feature`_ (4 scenarios)
...
_Source: `specs/apps/a-demo/be/gherkin/expenses/reporting.feature`_ (6 scenarios)
...
_Source: `specs/apps/a-demo/be/gherkin/expenses/attachments.feature`_ (10 scenarios)
...
- Coverage ≥ 90% (enforced by `rhino-cli test-coverage validate` against Kover JaCoCo XML output)
- All Nx mandatory targets present: `build`, `dev`, `start`, `test:quick`, `test:unit`,
  `test:integration`, `lint`
- No `typecheck` target needed — Kotlin compiler enforces null safety at compile time, and
  `build` already invokes the compiler
- ktfmt formatting passes with zero changes (`./gradlew ktfmtCheck`)
- detekt passes with zero violations (`./gradlew detekt`)
- Kotlin compiler null safety enforced — no platform type leaks (`@NotNull`/`@Nullable` from Java
  dependencies must be wrapped)
- All 76 Gherkin scenarios pass in integration tests
- All 76 Gherkin scenarios pass in E2E tests (via `a-demo-be-e2e`)
- Port 8201 (mutually exclusive with a-demo-be-java-springboot, a-demo-be-elixir-phoenix, and a-demo-be-fsharp-giraffe)
- Password constraints: min 12 chars, must include special character

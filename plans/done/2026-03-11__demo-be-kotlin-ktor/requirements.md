# Requirements: demo-be-kotlin-ktor
...
- **Integration tests** (`test:integration`): Ktor `testApplication {}` in-process HTTP testing
  ...
  cache. Matches the monorepo convention established by `demo-be-java-springboot` (MockMvc +
  InMemoryDataStore), `demo-be-elixir-phoenix` (InMemoryStore), and `demo-be-fsharp-giraffe` (TestServer + SQLite).
- **E2E tests** (`test:e2e`): Playwright via `demo-be-e2e` against the live Dockerised stack at
...
- Port 8201 (mutually exclusive with demo-be-java-springboot, demo-be-elixir-phoenix, and demo-be-fsharp-giraffe)

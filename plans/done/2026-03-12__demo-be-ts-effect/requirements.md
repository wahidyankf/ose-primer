# Requirements: a-demo-be-ts-effect
...
- **Integration tests** (`test:integration`): Cucumber.js step definitions using the Effect TS
  HTTP client against an in-process server backed by `@effect/sql-sqlite-node` in-memory
  database — fully in-process, no external services, safe to cache. Matches the monorepo
  convention established by `a-demo-be-java-springboot` (MockMvc + InMemoryDataStore), `a-demo-be-python-fastapi`
  (TestClient + SQLite in-memory), and `a-demo-be-rust-axum` (cucumber + Tower TestClient +
  in-memory stores).
...
- Prettier formatting passes with zero changes (`npx prettier --check .`)
- oxlint passes with zero violations (`npx oxlint .`)
- `tsc --noEmit` passes with zero type errors (strict mode)

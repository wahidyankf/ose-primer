...
- [x] **1.1** Create `apps/rhino-cli/cmd/testable.go` — package-level function variables for all
...
- [x] **1.2** Create `apps/rhino-cli/cmd/testable_mock_test.go` — reusable mock utilities:
      `mockFileInfo`, `mockFS` struct with `addFile`/`addDir`/`installMocks` methods, `mockCommandRunner`
- [x] **1.3** Create `apps/rhino-cli/cmd/steps_common_test.go` — shared step text regex constants
      used by both unit and integration tests (e.g., `stepExitsSuccessfully`,
      `stepExitsWithFailure`, `stepOutputIsValidJSON`)
- [x] **1.4** Verify `go test ./...` still passes with no functional changes (new files are test
      helpers only)
...
- [x] **2.1** Migrate `cmd/helpers.go` — `findGitRoot()` uses `osGetwd`, `osStat`
- [x] **2.2** Migrate `cmd/doctor.go` — delegates to `internal/doctor.CheckAll()`; wire
...
- [x] **2.3** Migrate `cmd/test_coverage_validate.go` — verify call sites; delegates to
      `internal/`; wire `osReadFile`, `osStat` at the cmd or options boundary
- [x] **2.4** Migrate `cmd/test_coverage_merge.go` — verify call sites; wire `osReadFile`,
      `osWriteFile` at the cmd or options boundary
- [x] **2.5** Migrate `cmd/test_coverage_diff.go` — verify call sites; wire `osReadFile`,
      `execCommand` at the cmd or options boundary
- [x] **2.6** Migrate `cmd/agents_sync.go` — verify call sites; wire `osReadFile`, `osWriteFile`,
      `osMkdirAll`, `walkDir`, `osStat` at the cmd or options boundary
- [x] **2.7** Migrate `cmd/agents_validate_claude.go` — verify call sites; wire `osReadFile`,
      `walkDir`, `osStat` at the cmd or options boundary
- [x] **2.8** Migrate `cmd/agents_validate_sync.go` — verify call sites; wire `osReadFile`,
      `walkDir`, `osStat` at the cmd or options boundary
- [x] **2.9** Migrate `cmd/docs_validate_links.go` — verify call sites; wire `osReadFile`,
      `walkDir` at the cmd or options boundary
- [x] **2.10** Migrate `cmd/docs_validate_naming.go` — verify call sites; wire `osReadFile`,
      `walkDir` at the cmd or options boundary
- [x] **2.11** Migrate `cmd/contracts_java_clean_imports.go` — verify call sites; wire
      `osReadFile`, `osWriteFile`, `walkDir` at the cmd or options boundary
- [x] **2.12** Migrate `cmd/contracts_dart_scaffold.go` — verify call sites; wire `osReadFile`,
      `osWriteFile`, `walkDir`, `osMkdirAll` at the cmd or options boundary
- [x] **2.13** Migrate `cmd/java_validate_annotations.go` — verify call sites; wire `osReadFile`,
      `walkDir` at the cmd or options boundary
- [x] **2.14** Migrate `cmd/spec_coverage_validate.go` — verify call sites; wire `osReadFile`,
      `walkDir`, `osStat` at the cmd or options boundary
- [x] **2.15** Migrate `cmd/git_pre_commit.go` — verify call sites; wire `osReadFile`,
      `execCommand`, `osGetwd` at the cmd or options boundary
- [x] **2.16** Run `nx run rhino-cli:test:quick` — verify all existing tests still pass (migration
      is a refactor, no behavior change)
- [x] **2.17** Run `nx run rhino-cli:test:integration` — verify integration tests still pass
...
- [x] **3.1** Rewrite `cmd/doctor_test.go` — godog consuming `doctor.feature` with mocked
      filesystem and tool detection; keep `TestDoctorCommand_Initialization` as non-BDD
- [x] **3.2** Rewrite `cmd/test_coverage_validate_test.go` — godog consuming
      `test-coverage-validate.feature` with mocked coverage file reads
- [x] **3.3** Create `cmd/test_coverage_merge_test.go` — godog consuming
      `test-coverage-merge.feature` with mocked file reads/writes (file did not exist before)
- [x] **3.4** Create `cmd/test_coverage_diff_test.go` — godog consuming
      `test-coverage-diff.feature` with mocked file reads and git diff (file did not exist before)
- [x] **3.5** Rewrite `cmd/agents_sync_test.go` — godog consuming `agents-sync.feature` with
      mocked filesystem for `.claude/`/`.opencode/` directories
- [x] **3.6** Rewrite `cmd/agents_validate_claude_test.go` — godog consuming
      `agents-validate-claude.feature` with mocked YAML file reads
- [x] **3.7** Rewrite `cmd/agents_validate_sync_test.go` — godog consuming
      `agents-validate-sync.feature` with mocked directory walks
- [x] **3.8** Rewrite `cmd/docs_validate_links_test.go` — godog consuming
      `docs-validate-links.feature` with mocked markdown file reads
- [x] **3.9** Rewrite `cmd/docs_validate_naming_test.go` — godog consuming
      `docs-validate-naming.feature` with mocked file listings
- [x] **3.10** Rewrite `cmd/contracts_java_clean_imports_test.go` — godog consuming
      `contracts-java-clean-imports.feature` with mocked Java source reads/writes
- [x] **3.11** Rewrite `cmd/contracts_dart_scaffold_test.go` — godog consuming
      `contracts-dart-scaffold.feature` with mocked Dart source reads/writes
- [x] **3.12** Rewrite `cmd/java_validate_annotations_test.go` — godog consuming
      `java-validate-annotations.feature` with mocked package-info reads
- [x] **3.13** Rewrite `cmd/spec_coverage_validate_test.go` — godog consuming
      `spec-coverage-validate.feature` with mocked feature file + test file reads
- [x] **3.14** Rewrite `cmd/git_pre_commit_test.go` — godog consuming `git-pre-commit.feature`
      with mocked git operations and staged file reads
- [x] **3.15** Verify `cmd/root_test.go` and `cmd/helpers_test.go` — keep as non-BDD (no
      Gherkin specs for root/helpers); update to use mocked filesystem where applicable
- [x] **3.16** Run `nx run rhino-cli:test:quick` — all unit tests pass with >=90% coverage

## Phase 4: Verify Integration Tests

Integration tests should already be correct. Verify and adjust if needed.

- [x] **4.1** Create `cmd/test_coverage_diff.integration_test.go` — godog consuming
      `test-coverage-diff.feature` with real filesystem (this integration test does not exist yet)
- [x] **4.2** Audit all `*.integration_test.go` files (now 14) — confirm they use real filesystem
      (`os.MkdirTemp`, `os.WriteFile`, etc.) and NOT the mocked package-level vars
- [x] **4.3** Ensure integration tests use `os.MkdirTemp("", "rhino-*")` consistently for
      `/tmp` fixture creation
- [x] **4.4** Verify integration test `Before` hooks do NOT install mocks (leave `osReadFile` etc.
      pointing to real `os.ReadFile`)
- [x] **4.5** Run `nx run rhino-cli:test:integration` — all integration tests pass

## Phase 5: Update Nx Configuration

- [x] **5.1** Update `apps/rhino-cli/project.json` — add `specs/apps/rhino-cli/**/*.feature` to
      `test:unit` and `test:quick` inputs (ensures cache invalidation on spec changes)
- [x] **5.2** Verify `test:quick` command does not need changes (already runs `go test ./...` which
      picks up godog in unit tests automatically)
- [x] **5.3** Run full quality gate: `nx affected -t typecheck lint test:quick` — passes
...
- [x] **6.5** Update `specs/apps/rhino-cli/README.md`:
...
- [x] **6.6** Update `specs/apps/ayokoding-cli/README.md`:
...
- [x] **6.7** Update `specs/apps/oseplatform-cli/README.md`:
  - Same updates as ayokoding-cli specs README
- [x] **6.8** Update `apps/rhino-cli/README.md`:
...
- [x] **6.9** Update `apps/ayokoding-cli/README.md`:
  - Update testing section to describe dual-consumption architecture
- [x] **6.10** Update `apps/oseplatform-cli/README.md`:
  - Update testing section to describe dual-consumption architecture
...
- [x] **7.1** Create `apps/ayokoding-cli/cmd/testable.go` — package-level vars: `checkLinksFn`
      (wrapping `links.CheckLinks`), `osGetwd`, `osStat`, `osChdir`, plus any other direct
      `os.*` calls in `links_check.go`
- [x] **7.2** Create `apps/ayokoding-cli/cmd/testable_mock_test.go` — mock helpers for mocked
      link check results and filesystem operations
- [x] **7.3** Migrate `apps/ayokoding-cli/cmd/links_check.go` — replace `links.CheckLinks()`
      with `checkLinksFn()`, replace direct `os.*` calls with package-level vars
- [x] **7.4** Verify existing tests still pass: `nx run ayokoding-cli:test:quick`
- [x] **7.5** Rewrite `apps/ayokoding-cli/cmd/links_check_test.go` — godog consuming
...
- [x] **7.6** Verify `apps/ayokoding-cli/cmd/root_test.go` — keep as non-BDD (no Gherkin
      specs for root command); update to use mocked `osExit` consistently
- [x] **7.7** Audit `apps/ayokoding-cli/cmd/links-check.integration_test.go` — confirm it
      uses real filesystem and does NOT install mocks; adjust if needed
- [x] **7.8** Update `apps/ayokoding-cli/project.json` — add
      `{workspaceRoot}/specs/apps/ayokoding-cli/**/*.feature` to `test:unit` and `test:quick`
      inputs
- [x] **7.9** Run `nx run ayokoding-cli:test:quick` — unit tests pass with >=90% coverage
- [x] **7.10** Run `nx run ayokoding-cli:test:integration` — integration tests pass
...
- [x] **8.1** Create `apps/oseplatform-cli/cmd/testable.go` — package-level vars: `checkLinksFn`
      (wrapping `links.CheckLinks`), keep existing `outputLinksJSONFn`, add `osGetwd`, `osStat`,
      `osChdir`, plus any other direct `os.*` calls
- [x] **8.2** Create `apps/oseplatform-cli/cmd/testable_mock_test.go` — mock helpers
- [x] **8.3** Migrate `apps/oseplatform-cli/cmd/links_check.go` — replace `links.CheckLinks()`
      with `checkLinksFn()`, replace direct `os.*` calls with package-level vars; preserve
      existing `outputLinksJSONFn` injection
- [x] **8.4** Verify existing tests still pass: `nx run oseplatform-cli:test:quick`
- [x] **8.5** Rewrite `apps/oseplatform-cli/cmd/links_check_test.go` — godog consuming
...
- [x] **8.6** Verify `apps/oseplatform-cli/cmd/root_test.go` — keep as non-BDD; update to
      use mocked `osExit` consistently
- [x] **8.7** Audit `apps/oseplatform-cli/cmd/links-check.integration_test.go` — confirm
      real filesystem usage; ensure `checkLinksFn` and `outputLinksJSONFn` are reset to real
      implementations in `before` hook
- [x] **8.8** Update `apps/oseplatform-cli/project.json` — add
      `{workspaceRoot}/specs/apps/oseplatform-cli/**/*.feature` to `test:unit` and `test:quick`
      inputs
- [x] **8.9** Run `nx run oseplatform-cli:test:quick` — unit tests pass with >=90% coverage
- [x] **8.10** Run `nx run oseplatform-cli:test:integration` — integration tests pass

## Phase 9: Cross-App Validation

- [x] **9.1** Run full quality gate for all three CLI apps:
      `nx run-many -t test:quick test:integration -p rhino-cli ayokoding-cli oseplatform-cli`
- [x] **9.2** Run `nx affected -t typecheck lint test:quick` — all affected projects pass
- [x] **9.3** Verify spec-coverage for all three apps:
...
- [x] All Gherkin scenarios pass at unit level (godog, mocked deps)
- [x] All Gherkin scenarios pass at integration level (godog, real fs)
- [x] Unit test coverage >=90% for rhino-cli
- [x] No real filesystem I/O in unit tests (verified by code review — no `os.MkdirTemp`,
      `os.WriteFile`, `os.Chdir` in `*_test.go` step definitions)
- [x] Integration tests use real filesystem (`/tmp` fixtures)
- [x] Integration tests skip gracefully when optional tools are unavailable
- [x] `test:quick` passes (includes unit-level Gherkin)
- [x] `test:integration` passes
- [x] `nx affected -t typecheck lint test:quick` passes

### Configuration

- [x] `project.json` `test:unit` inputs include `specs/apps/rhino-cli/**/*.feature`
- [x] `project.json` `test:quick` inputs include `specs/apps/rhino-cli/**/*.feature`
- [x] Cache invalidates when Gherkin specs change (verified by modifying a `.feature` file
      and confirming `test:unit` re-runs)

### Documentation

- [x] `three-level-testing-standard.md` CLI row updated
- [x] `bdd-spec-test-mapping.md` CLI section updated for dual consumption
- [x] `nx-targets.md` CLI targets and inputs updated
- [x] `CLAUDE.md` CLI testing descriptions updated
- [x] `specs/apps/rhino-cli/README.md` dual consumption documented
- [x] `specs/apps/ayokoding-cli/README.md` dual consumption documented
- [x] `specs/apps/oseplatform-cli/README.md` dual consumption documented
- [x] `apps/rhino-cli/README.md` testing section updated
- [x] `apps/ayokoding-cli/README.md` testing section updated
- [x] `apps/oseplatform-cli/README.md` testing section updated

### All CLI Apps

- [x] `ayokoding-cli` unit tests consume Gherkin with mocked `checkLinksFn`
- [x] `ayokoding-cli` integration tests use real filesystem
- [x] `ayokoding-cli` coverage >=90%
- [x] `oseplatform-cli` unit tests consume Gherkin with mocked `checkLinksFn`
- [x] `oseplatform-cli` integration tests use real filesystem
- [x] `oseplatform-cli` coverage >=90%
- [x] All three CLI apps pass:
      `nx run-many -t test:quick test:integration -p rhino-cli ayokoding-cli oseplatform-cli`

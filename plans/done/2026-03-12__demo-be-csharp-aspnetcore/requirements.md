# Requirements: a-demo-be-csharp-aspnetcore
...
- **Integration tests** (`test:integration`): Reqnroll step definitions with
  ...
  monorepo convention established by `a-demo-be-java-springboot` (MockMvc + InMemoryDataStore),
  `a-demo-be-elixir-phoenix` (InMemoryStore contexts), `a-demo-be-fsharp-giraffe` (TickSpec + WebApplicationFactory +
  SQLite in-memory), `a-demo-be-golang-gin` (Godog + httptest), `a-demo-be-python-fastapi` (pytest-bdd +
  TestClient), `a-demo-be-rust-axum` (Cucumber + Tower TestClient), and `a-demo-be-kotlin-ktor`

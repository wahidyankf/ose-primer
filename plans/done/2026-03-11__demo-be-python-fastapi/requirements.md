# Requirements: a-demo-be-python-fastapi
...
- **Integration tests** (`test:integration`): FastAPI `TestClient` (in-process — no live server
  needed) with SQLAlchemy bound to a SQLite in-memory database — fully in-process, no external
  services, safe to cache. Matches the monorepo convention established by `a-demo-be-java-springboot`
  (MockMvc + InMemoryDataStore), `a-demo-be-elixir-phoenix` (InMemoryStore contexts), and `a-demo-be-fsharp-giraffe`
  (ASP.NET Core TestServer + SQLite in-memory).
...
- Ruff formatting passes with zero changes (`uv run ruff format --check .`)
- Ruff linting passes with zero violations (`uv run ruff check .`)
- Pyright strict mode passes with zero errors (`uv run pyright`)

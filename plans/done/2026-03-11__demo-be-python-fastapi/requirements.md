# Requirements: demo-be-python-fastapi
...
- **Integration tests** (`test:integration`): FastAPI `TestClient` (in-process — no live server
  ...
  services, safe to cache. Matches the monorepo convention established by `demo-be-java-springboot`
  (MockMvc + InMemoryDataStore), `demo-be-elixir-phoenix` (InMemoryStore contexts), and `demo-be-fsharp-giraffe`

# Requirements: a-demo-be-golang-gin
...
- **Integration tests** (`test:integration`): Godog BDD runner + Gin test server + GORM SQLite
  in-memory — fully in-process, no external services, safe to cache. Matches the monorepo
  convention established by `a-demo-be-java-springboot` (MockMvc + InMemoryDataStore), `a-demo-be-elixir-phoenix`
  (InMemoryStore contexts), and `a-demo-be-fsharp-giraffe` (TestServer + SQLite).
...
- No `typecheck` target required — Go compiler enforces types via `build`; `go vet` is included
  in golangci-lint
- `gofmt` formatting passes with zero changes (enforced by pre-commit lint-staged)
- `golangci-lint` passes with zero violations (exhaustive, errcheck, staticcheck, govet)
- All 76 Gherkin scenarios pass in integration tests (Godog)
...
- Goroutine safety: in-memory test store protected by `sync.RWMutex`
- Error handling: all `error` return values checked (enforced by errcheck linter)

# Requirements: demo-be-golang-gin
...
- **Integration tests** (`test:integration`): Godog BDD runner + Gin test server + GORM SQLite
  ...
  convention established by `demo-be-java-springboot` (MockMvc + InMemoryDataStore), `demo-be-elixir-phoenix`
  (InMemoryStore contexts), and `demo-be-fsharp-giraffe` (TestServer + SQLite).

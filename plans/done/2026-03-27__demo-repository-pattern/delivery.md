...
### Phase 1: a-demo-be-python-fastapi (Python — smallest diff)
...
- [x] Create `src/a_demo_be_python_fastapi/infrastructure/protocols.py` with Protocol classes:
...
- [x] Create `src/a_demo_be_python_fastapi/infrastructure/refresh_token_repository.py` — extract
      RefreshToken DB logic from `src/a_demo_be_python_fastapi/routers/auth.py` and
      `src/a_demo_be_python_fastapi/routers/tokens.py`
- [x] Update `src/a_demo_be_python_fastapi/infrastructure/repositories.py` — ensure method signatures
      conform to Protocols
- [x] Update `src/a_demo_be_python_fastapi/dependencies.py` — type-hint return values as Protocol
      types, add `get_refresh_token_repo` provider (target is `dependencies.py` in the package
      root, not `auth/dependencies.py`)
- [x] Update `src/a_demo_be_python_fastapi/routers/auth.py` and
      `src/a_demo_be_python_fastapi/routers/tokens.py` — replace inline RefreshToken Session calls
...
- [x] Verify `nx run a-demo-be-python-fastapi:typecheck` passes
- [x] Verify `nx run a-demo-be-python-fastapi:lint` passes
- [x] Verify `nx run a-demo-be-python-fastapi:test:quick` passes (unit tests + coverage >= 90%)
- [x] Verify `nx run a-demo-be-python-fastapi:test:integration` passes (real Postgres)
- [x] Commit: `refactor(a-demo-be-python-fastapi): add Protocol abstractions for repository pattern`

### Phase 2: a-demo-be-clojure-pedestal (Clojure — moderate diff)
...
- [x] Create `src/a_demo_be_cjpd/db/protocols.clj` with protocols: `UserRepo`, `ExpenseRepo`,
      `AttachmentRepo`, `TokenRepo`
- [x] Create `src/a_demo_be_cjpd/db/jdbc_user_repo.clj` — `defrecord JdbcUserRepo [ds]`
      implementing `UserRepo` (wrap existing `user_repo.clj` functions)
- [x] Create `src/a_demo_be_cjpd/db/jdbc_expense_repo.clj` — `defrecord JdbcExpenseRepo [ds]`
      implementing `ExpenseRepo`
- [x] Create `src/a_demo_be_cjpd/db/jdbc_attachment_repo.clj` — `defrecord JdbcAttachmentRepo [ds]`
      implementing `AttachmentRepo`
- [x] Create `src/a_demo_be_cjpd/db/jdbc_token_repo.clj` — `defrecord JdbcTokenRepo [ds]`
...
- [x] Update `src/a_demo_be_cjpd/server.clj` — create `Jdbc*Repo` records and inject into Pedestal
      context map
- [x] Create `test/a_demo_be_cjpd/in_memory_repos.clj` — atom-backed `defrecord` implementations
...
- [x] Verify `nx run a-demo-be-clojure-pedestal:lint` passes
- [x] Verify `nx run a-demo-be-clojure-pedestal:test:quick` passes (unit tests + coverage >= 90%)
- [x] Verify `nx run a-demo-be-clojure-pedestal:test:integration` passes (real Postgres)
- [x] Commit: `refactor(a-demo-be-clojure-pedestal): add defprotocol abstractions for repository pattern`

### Phase 3: a-demo-be-rust-axum (Rust — larger diff)
...
- [x] Verify `nx run a-demo-be-rust-axum:lint` passes
- [x] Verify `nx run a-demo-be-rust-axum:test:quick` passes (unit tests + coverage >= 90%)
- [x] Verify `nx run a-demo-be-rust-axum:test:integration` passes (real Postgres)
- [x] Commit: `refactor(a-demo-be-rust-axum): add trait abstractions for repository pattern`

### Phase 4: a-demo-be-fsharp-giraffe (F# — largest diff)
...
- [x] Verify `nx run a-demo-be-fsharp-giraffe:typecheck` passes
- [x] Verify `nx run a-demo-be-fsharp-giraffe:lint` passes
- [x] Verify `nx run a-demo-be-fsharp-giraffe:test:quick` passes (unit tests + coverage >= 90%)
- [x] Verify `nx run a-demo-be-fsharp-giraffe:test:integration` passes (real Postgres)
- [x] Commit: `refactor(a-demo-be-fsharp-giraffe): add function-record abstractions for repository pattern`

## Final Validation

- [x] Run `nx run-many -t typecheck --projects=a-demo-be-python-fastapi,a-demo-be-clojure-pedestal,a-demo-be-rust-axum,a-demo-be-fsharp-giraffe` — all pass (Clojure typecheck blocked by pre-existing codegen build failure)
- [x] Run `nx run-many -t lint --projects=a-demo-be-python-fastapi,a-demo-be-clojure-pedestal,a-demo-be-rust-axum,a-demo-be-fsharp-giraffe` — all pass
- [x] Run `nx run-many -t test:quick --projects=a-demo-be-python-fastapi,a-demo-be-clojure-pedestal,a-demo-be-rust-axum,a-demo-be-fsharp-giraffe` — all pass
- [x] Run `nx run-many -t test:integration --projects=a-demo-be-python-fastapi,a-demo-be-clojure-pedestal,a-demo-be-rust-axum,a-demo-be-fsharp-giraffe` — all 4 PASS in GitHub CI

...
1. **Commit by theme, not by phase** — Group changes that belong together. For example, when working on `a-demo-be-java-springboot`, commit unit test step definitions separately from docker-compose infrastructure. This keeps each commit reviewable and the diff small.
...
5. **Example commit sequence for one backend**:
   - `test(a-demo-be-java-springboot): add unit-level Gherkin step definitions` → push
   - `refactor(a-demo-be-java-springboot): replace MockMvc with direct service calls in integration tests` → push
   - `ci(a-demo-be-java-springboot): add docker-compose.integration.yml and Dockerfile.integration` → push
   - `chore(a-demo-be-java-springboot): update project.json targets and README` → push
   - `test(a-demo-be-java-springboot): verify coverage ≥90% from unit tests` → push
...
- [x] **1.4 Update `specs/apps/a-demo/be/README.md`**
...
Implement one backend at a time. Each backend follows 7 steps. The first backend (`a-demo-be-java-springboot`) sets the reference pattern. Complete one backend fully before starting the next.

### 2.1 `a-demo-be-java-springboot` (reference implementation)
...
- [x] **2.1.1 Unit test step definitions**
  ...
  - [x] Commit: `test(a-demo-be-java-springboot): add unit-level Gherkin step definitions` → push
- [x] **2.1.2 Integration test refactor**
  ...
  - [x] Commit: `ci(a-demo-be-java-springboot): add docker-compose integration infrastructure` → push
...
- [x] **2.1.5 Update `project.json`**
  ...
  - [x] Add caching inputs: `specs/apps/a-demo/be/gherkin/**/*.feature` for `test:unit` and `test:quick`
- [x] **2.1.6 Update `apps/a-demo-be-java-springboot/README.md`**
...
- [x] **2.1.7 Verify coverage**
  - [x] Run `nx run a-demo-be-java-springboot:test:unit` — 76/76 pass, coverage file generated
  - [x] Run `rhino-cli test-coverage validate` — 95.20% ≥ 90% threshold
  - [x] Run `nx run a-demo-be-java-springboot:test:integration` — 76/76 pass with real PostgreSQL
  - [x] Commit: `ci(a-demo-be-java-springboot): add docker-compose integration infrastructure and update README` → push

### 2.2 `a-demo-be-kotlin-ktor` (JVM — replace SQLite → PostgreSQL)
...
- [x] **2.2.7** Verify: 91.29% coverage from unit tests, all scenarios pass at both levels
  - [x] Commit: `test(a-demo-be-kotlin-ktor): add unit-level step definitions and docker-compose integration` → push

### 2.3 `a-demo-be-java-vertx` (JVM — add PostgreSQL)
...
- [x] **2.3.7** Verify: 92.36% coverage from unit tests, all 76 scenarios pass
  - [x] Commit: `test(a-demo-be-java-vertx): add unit-level step definitions and docker-compose integration` → push

### 2.4 `a-demo-be-fsharp-giraffe` (.NET — replace SQLite → PostgreSQL)
...
- [x] **2.4.2** Refactor integration steps — TestWebApplicationFactory detects DATABASE_URL for PostgreSQL vs SQLite
  - [x] Commit: `test(a-demo-be-fsharp-giraffe): add docker-compose integration and update targets` → push
...
### 2.5 `a-demo-be-csharp-aspnetcore` (.NET — replace SQLite → PostgreSQL)
...
- [x] **2.5.2** TestWebApplicationFactory detects DATABASE_URL for PostgreSQL vs SQLite
  - [x] Commit: `test(a-demo-be-csharp-aspnetcore): add docker-compose integration and update targets` → push
...
### 2.6 `a-demo-be-golang-gin` (Go — add PostgreSQL)
...
- [x] **2.6.2** Create integration PostgreSQL steps in `internal/integration_pg/`
  - [x] Commit: `test(a-demo-be-golang-gin): add unit BDD steps, integration PostgreSQL, and docker-compose` → push
...
### 2.7 `a-demo-be-ts-effect` (Node.js — add PostgreSQL)
...
- [x] **2.7.2** Refactor integration steps — remove HTTP client layer, call service functions directly, add PostgreSQL (pg or drizzle)
  - [x] Commit: `test(a-demo-be-ts-effect): add unit steps and refactor integration to PostgreSQL` → push
...
- [x] **2.7.4** Create `Dockerfile.integration` (Node.js 24 base image)
  - [x] Commit: `ci(a-demo-be-ts-effect): add docker-compose and Dockerfile for integration tests` → push
...
- [x] **2.7.6** Update README — test architecture, commands, docker-compose docs
  - [x] Commit: `chore(a-demo-be-ts-effect): update project.json targets and README` → push
- [x] **2.7.7** Verify: 91.97% coverage from unit tests, 76 scenarios pass at unit level

### 2.8 `a-demo-be-python-fastapi` (Python — add PostgreSQL)
...
- [x] **2.8.2** Refactor integration steps — remove TestClient HTTP layer, call service functions directly, add PostgreSQL (asyncpg or psycopg)
  - [x] Commit: `test(a-demo-be-python-fastapi): add unit steps and refactor integration to PostgreSQL` → push
...
- [x] **2.8.4** Create `Dockerfile.integration` (Python 3.13 + uv base image)
  - [x] Commit: `ci(a-demo-be-python-fastapi): add docker-compose and Dockerfile for integration tests` → push
...
- [x] **2.8.6** Update README — test architecture, commands, docker-compose docs
  - [x] Commit: `chore(a-demo-be-python-fastapi): update project.json targets and README` → push
- [x] **2.8.7** Verify: 94.62% coverage from unit tests, 108 unit tests pass

### 2.9 `a-demo-be-clojure-pedestal` (JVM/Clojure — replace SQLite → PostgreSQL)
...
- [x] **2.9.2** Refactor integration steps — remove clj-http HTTP layer, call handler functions directly, replace SQLite with PostgreSQL (next.jdbc)
  - [x] Commit: `test(a-demo-be-clojure-pedestal): add docker-compose integration and update test targets` → push
...
### 2.10 `a-demo-be-elixir-phoenix` (Elixir/OTP — add PostgreSQL via Ecto)
...
- [x] **2.10.2** Refactor integration steps — remove ConnCase HTTP layer, call context functions directly, add PostgreSQL via Ecto sandbox
  - [x] Commit: `test(a-demo-be-elixir-phoenix): add unit steps and refactor integration to PostgreSQL` → push
...
- [x] **2.10.4** Create `Dockerfile.integration` (elixir:1.19-otp-27-alpine + build-base)
  - [x] Commit: `ci(a-demo-be-elixir-phoenix): add docker-compose and Dockerfile for integration tests` → push
...
- [x] **2.10.6** Update README — test architecture, commands, docker-compose docs
  - [x] Commit: `chore(a-demo-be-elixir-phoenix): update project.json targets and README` → push
- [x] **2.10.7** Verify: 91.67% coverage from unit tests, all scenarios pass

### 2.11 `a-demo-be-rust-axum` (Rust — add PostgreSQL)
...
- [x] **2.11.2** Refactor integration steps — remove Tower TestClient HTTP layer, call handler functions directly, add PostgreSQL (sqlx)
  - [x] Commit: `test(a-demo-be-rust-axum): add unit steps and refactor integration to PostgreSQL` → push
...
- [x] **2.11.4** Create `Dockerfile.integration` (rust:latest base image)
  - [x] Commit: `ci(a-demo-be-rust-axum): add docker-compose and Dockerfile for integration tests` → push
...
- [x] **2.11.6** Update README — test architecture, commands, docker-compose docs
  - [x] Commit: `chore(a-demo-be-rust-axum): update project.json targets and README` → push
...
- [x] **3.12 `a-demo-be-e2e`** — already compliant, has `test:e2e` and `test:quick` (bddgen)
...
- [x] **4.1 Update `main-ci.yml`**
  ...
  - [x] Fix `a-demo-be-java-vertx` coverage path from `jacoco-integration` to `jacoco`
...
- [x] **4.4 Verify E2E workflows (12 files)**
  - [x] Confirm all `e2e-a-demo-be-*.yml` and `test-integration-e2e-organiclever-fe.yml` use cron `0 23 * * *` and `0 11 * * *` (WIB 06/18)
  - [x] Confirm no changes needed (already compliant)
...
- [x] **5.1 Demo-be backends — all three levels**
  - [x] Run `nx run-many -t test:unit --projects=a-demo-be-*` — all 11 backends pass
  - [ ] Run `nx run-many -t test:integration --projects=a-demo-be-*` — requires Docker (skipped in local verification; CI workflow handles this)
...
- [x] **5.3 Coverage**
  ...
  - [x] Verified `main-ci.yml` coverage uploads point to correct files (a-demo-be-java-vertx fixed from jacoco-integration to jacoco)
- [x] **5.4 Nx caching**

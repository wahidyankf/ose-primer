...
flowchart LR
    ...
    D -->|plan-establishment ref| F[plan-establishment-exec.md\nreferenced]
...
**Location**: Item 3 in the "Agent Organization" numbered list (currently references
`plan-maker`, `plan-checker`, `plan-execution-checker`, `plan-fixer` only).

**Replace with** (shown as plain text — paths are repo-root-relative as they appear in `AGENTS.md`):

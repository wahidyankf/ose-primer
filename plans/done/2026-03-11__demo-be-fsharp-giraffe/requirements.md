# Requirements: a-demo-be-fsharp-giraffe
...
- **Integration tests** (`test:integration`): ASP.NET Core TestServer + in-memory database
  (SQLite or EF Core InMemory) — fully in-process, no external services, safe to cache.
  Matches the monorepo convention established by `a-demo-be-java-springboot` (MockMvc + InMemoryDataStore)
  and `a-demo-be-elixir-phoenix` (InMemoryStore contexts).
...
- Fantomas formatting passes with zero changes (`dotnet fantomas --check`)
- FSharpLint passes with zero violations
- `<TreatWarningsAsErrors>true</TreatWarningsAsErrors>` — compiler catches all warnings
- All 76 Gherkin scenarios pass in integration tests
- All 76 Gherkin scenarios pass in E2E tests (via `a-demo-be-e2e`)
- Port 8201 (mutually exclusive with a-demo-be-java-springboot and a-demo-be-elixir-phoenix)

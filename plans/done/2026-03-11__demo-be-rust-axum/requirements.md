# Requirements: a-demo-be-rust-axum
...
- **Integration tests** (`test:integration`): Axum `tower::ServiceExt` in-process HTTP testing
  with SQLite in-memory via SQLx — no external services, safe to cache. Matches the monorepo
  convention established by `a-demo-be-java-springboot` (MockMvc + InMemoryDataStore), `a-demo-be-elixir-phoenix`
  (InMemoryStore contexts), and `a-demo-be-fsharp-giraffe` (ASP.NET Core TestServer + SQLite in-memory).
- **E2E tests** (`test:e2e`): Playwright via `a-demo-be-e2e` against the live Dockerised stack
...
- Coverage ≥ 90% (enforced by `rhino-cli test-coverage validate` on LCOV from cargo-llvm-cov)
- All Nx mandatory targets present: `build`, `dev`, `start`, `test:quick`, `test:unit`,
  `test:integration`, `lint`, `typecheck`
- `rustfmt` formatting passes with zero changes (`cargo fmt --check`)
- Clippy passes with zero warnings (`cargo clippy -- -D warnings`)
- `cargo build` compiles on stable Rust with zero warnings treated as errors
- All 76 Gherkin scenarios pass in integration tests (cucumber-rs)
- All 76 Gherkin scenarios pass in E2E tests (via `a-demo-be-e2e`)
- Port 8201 (mutually exclusive with a-demo-be-java-springboot, a-demo-be-elixir-phoenix, a-demo-be-fsharp-giraffe)
- Password constraints: min 12 chars, must include special character, must include uppercase
- Supported currencies: USD (2 decimal places), IDR (0 decimal places)
- All async handlers use `async fn` with Tokio runtime; no blocking calls on the async executor

# Delivery — Fix Mermaid Validation and Violations

## Environment Setup

- [x] Install dependencies: `npm install`
- [x] Converge the full polyglot toolchain: `npm run doctor -- --fix` (required — the
      `postinstall` hook runs `doctor || true` and silently tolerates drift)
- [x] Verify Go is available: `go version` (rhino-cli requires Go to run the validator)
- [x] Verify existing tests pass before making changes:
      `npx nx affected -t test:quick`

<!-- 2026-04-25 | Status: Done | All 19/19 tools OK; rhino-cli:test:quick passes at 90.02% -->

## Pre-Work

- [x] Run full audit and save as baseline:
      `go run ./apps/rhino-cli/main.go docs validate-mermaid 2>&1 | tee local-temp/mermaid-audit-baseline.txt`
- [x] Note counts (for your own reference — no expected value to match):
      `grep -c "^✗" local-temp/mermaid-audit-baseline.txt` # error lines
      `grep -c "^⚠" local-temp/mermaid-audit-baseline.txt` # warning lines
      `grep "^✗" local-temp/mermaid-audit-baseline.txt | grep -o '"[^"]*"' | sort -u | wc -l` # approx files
      These counts are your pre-Phase-0 baseline. They will change after Phase 0 because
      the direction-aware fix reclassifies LR diagrams. Historical reference numbers are
      in README.md Violation Baseline section.

<!-- 2026-04-25 | Status: Done | Baseline: 101 ✗ lines, 6 ⚠ lines, 235 total violations -->

## Phase 0 — Direction-Aware Validator + Threshold Update (rhino-cli)

### 0a — `validator.go` changes

- [x] Add `import "math"` to `validator.go`
- [x] Change `DefaultValidateOptions()`: `MaxWidth: 3 → 4`, `MaxDepth: 5 → math.MaxInt`
- [x] Add direction-aware `horizontal`/`vertical` selection in `ValidateBlocks` per
      tech-docs.md Phase 0 spec (switch on `diagram.Direction`, LR/RL → swap axes)

<!-- 2026-04-25 | Status: Done | Files: validator.go rewritten with math import, new defaults, direction-aware switch -->

### 0b — `validator_test.go` changes

- [x] Fix 3 tests broken by MaxWidth 3→4 (see tech-docs.md "Existing tests that break"):
  - `"width at limit+1 violation"` → switch to `span5depth3Source`
  - `"both exceeded warning only"` → use explicit `ValidateOptions{MaxWidth:3,MaxDepth:5}`
  - `"width only exceeded violation"` → use explicit opts or span=5 source
- [x] Add `"width exactly at new limit 4 no violation"` (span=4, TD, defaultOpts)
- [x] Add new LR direction test cases (6 cases per table in tech-docs.md Phase 0)
- [x] Add new test sources: `span5depth3Source`, `lrDepth6Span2Source`,
      `lrSpan5Depth2Source`, `lrDepth4Span6Source`

<!-- 2026-04-25 | Status: Done | Files: validator_test.go rewritten with fixed tests + 4 new sources + 6 direction-aware test cases -->

### 0c — `docs_validate_mermaid.go` CLI flag defaults

- [x] Change `--max-width` default: `3 → 4`
- [x] Change `--max-depth` default: `5 → 0` (document: 0 = no limit)
- [x] In `runValidateMermaid`: map `MaxDepth ≤ 0 → math.MaxInt` before building opts

<!-- 2026-04-25 | Status: Done | Files: docs_validate_mermaid.go — added math import, updated flag defaults and MaxDepth mapping -->

### 0d — Verify and commit

- [x] Run: `npx nx run rhino-cli:lint` → must pass
- [x] Run: `npx nx run rhino-cli:typecheck` → must pass
- [x] Run: `npx nx run rhino-cli:test:unit` → must pass
- [x] Run: `npx nx run rhino-cli:test:quick` → must pass (coverage ≥ 90%)
- [x] Commit: `fix(rhino-cli): make width_exceeded check direction-aware`
- [x] Re-audit: `go run ./apps/rhino-cli/main.go docs validate-mermaid 2>&1 | tee local-temp/mermaid-audit-phase0.txt`
- [x] Update Phase 1 batch file lists from the re-audit output
      (LR-only-tall files may disappear; new LR-deeply-chained files may appear;
      diagrams with old span=4 violations now pass)
- [x] Update the Violation Baseline table in README.md with new counts from Phase 0 re-audit

<!-- 2026-04-25 | Status: Done | lint/typecheck/test:unit/test:quick all pass (90.04%); committed; re-audit: 161 width_exceeded + 56 label_too_long = 217 violations, 100 files, 0 warnings; batch lists updated -->

> **Important**: The batch file lists below are provisional — based on the 2026-04-25
> pre-Phase-0 audit with the direction-blind validator. After step 0d re-audit, update
> each batch's file list to match `local-temp/mermaid-audit-phase0.txt`. Files that no
> longer appear in the re-audit output can be skipped; new files that appear must be added
> to the appropriate batch. Do not begin Batch 1 until the file lists are updated.

## Batch 1 — TypeScript (17 files — verified from Phase 0 re-audit)

Files: `docs/explanation/software-engineering/programming-languages/typescript/`

- [x] Fix `typescript/README.md`
- [x] Fix `typescript/anti-patterns.md`
- [x] Fix `typescript/best-practices.md`
- [x] Fix `typescript/concurrency-and-parallelism.md`
- [x] Fix `typescript/domain-driven-design.md`
- [x] Fix `typescript/error-handling.md`
- [x] Fix `typescript/finite-state-machine.md`
- [x] Fix `typescript/functional-programming.md`
- [x] Fix `typescript/idioms.md`
- [x] Fix `typescript/interfaces-and-types.md`
- [x] Fix `typescript/linting-and-formatting.md`
- [x] Fix `typescript/memory-management.md`
- [x] Fix `typescript/modules-and-dependencies.md`
- [x] Fix `typescript/performance.md`
- [x] Fix `typescript/security.md`
- [x] Fix `typescript/type-safety.md`
- [x] Fix `typescript/web-services.md`
- [x] Validate: `go run ./apps/rhino-cli/main.go docs validate-mermaid 2>&1 | grep "typescript/"` → no output
- [x] Commit: `fix(docs): fix mermaid violations in typescript/ docs (batch 1/10)`

<!-- 2026-04-25 | Status: Done | All 17 files fixed; direction flip primary strategy; zero violations confirmed -->

## Batch 2 — Python (13 files — verified from Phase 0 re-audit)

Files: `docs/explanation/software-engineering/programming-languages/python/`

- [x] Fix `python/anti-patterns.md`
- [x] Fix `python/best-practices.md`
- [x] Fix `python/classes-and-protocols.md`
- [x] Fix `python/concurrency-and-parallelism.md`
- [x] Fix `python/domain-driven-design.md`
- [x] Fix `python/error-handling.md`
- [x] Fix `python/functional-programming.md`
- [x] Fix `python/idioms.md`
- [x] Fix `python/memory-management.md`
- [x] Fix `python/modules-and-dependencies.md`
- [x] Fix `python/performance.md`
- [x] Fix `python/type-safety.md`
- [x] Fix `python/web-services.md`
- [x] Validate: `go run ./apps/rhino-cli/main.go docs validate-mermaid 2>&1 | grep "python/"` → no output
- [x] Commit: `fix(docs): fix mermaid violations in python/ docs (batch 2/10)`

<!-- 2026-04-25 | Status: Done | All 13 files fixed; zero violations confirmed -->

## Batch 3 — Go (12 files — verified from Phase 0 re-audit)

Files: `docs/explanation/software-engineering/programming-languages/golang/`

- [x] Fix `golang/README.md`
- [x] Fix `golang/api-standards.md`
- [x] Fix `golang/code-quality-standards.md`
- [x] Fix `golang/concurrency-standards.md`
- [x] Fix `golang/ddd-standards.md`
- [x] Fix `golang/dependency-standards.md`
- [x] Fix `golang/design-patterns.md`
- [x] Fix `golang/error-handling-standards.md`
- [x] Fix `golang/performance-standards.md`
- [x] Fix `golang/security-standards.md`
- [x] Fix `golang/testing-standards.md`
- [x] Fix `golang/type-safety-standards.md`
- [x] Validate: `go run ./apps/rhino-cli/main.go docs validate-mermaid 2>&1 | grep "golang/"` → no output
- [x] Commit: `fix(docs): fix mermaid violations in golang/ docs (batch 3/10)`

<!-- 2026-04-25 | Status: Done | All 12 files fixed; zero violations confirmed -->

## Batch 4 — JVM Spring Boot (10 files — verified from Phase 0 re-audit)

Files: `docs/explanation/software-engineering/platform-web/tools/jvm-spring-boot/`

- [x] Fix `jvm-spring-boot/README.md`
- [x] Fix `jvm-spring-boot/configuration.md`
- [x] Fix `jvm-spring-boot/data-access.md`
- [x] Fix `jvm-spring-boot/dependency-injection.md`
- [x] Fix `jvm-spring-boot/domain-driven-design.md`
- [x] Fix `jvm-spring-boot/observability.md`
- [x] Fix `jvm-spring-boot/performance.md`
- [x] Fix `jvm-spring-boot/rest-apis.md`
- [x] Fix `jvm-spring-boot/security.md`
- [x] Fix `jvm-spring-boot/testing.md`
- [x] Validate: `go run ./apps/rhino-cli/main.go docs validate-mermaid 2>&1 | grep "jvm-spring-boot/"` → no output
- [x] Commit: `fix(docs): fix mermaid violations in jvm-spring-boot/ docs (batch 4/10)`

<!-- 2026-04-25 | Status: Done | All 10 files fixed; zero violations confirmed -->

## Batch 5 — Elixir Phoenix (10 files — verified from Phase 0 re-audit)

Files: `docs/explanation/software-engineering/platform-web/tools/elixir-phoenix/`

- [x] Fix `elixir-phoenix/channels.md`
- [x] Fix `elixir-phoenix/configuration.md`
- [x] Fix `elixir-phoenix/contexts.md`
- [x] Fix `elixir-phoenix/data-access.md`
- [x] Fix `elixir-phoenix/deployment.md`
- [x] Fix `elixir-phoenix/liveview.md`
- [x] Fix `elixir-phoenix/observability.md`
- [x] Fix `elixir-phoenix/performance.md`
- [x] Fix `elixir-phoenix/rest-apis.md`
- [x] Fix `elixir-phoenix/testing.md`
- [x] Validate: `go run ./apps/rhino-cli/main.go docs validate-mermaid 2>&1 | grep "elixir-phoenix/"` → no output
- [x] Commit: `fix(docs): fix mermaid violations in elixir-phoenix/ docs (batch 5/10)`

<!-- 2026-04-25 | Status: Done | All 10 files fixed; zero violations confirmed -->

## Batch 6 — React (8 files — verified from Phase 0 re-audit)

Files: `docs/explanation/software-engineering/platform-web/tools/fe-react/`

- [x] Fix `fe-react/README.md`
- [x] Fix `fe-react/component-architecture.md`
- [x] Fix `fe-react/data-fetching.md`
- [x] Fix `fe-react/hooks.md`
- [x] Fix `fe-react/performance.md`
- [x] Fix `fe-react/routing.md`
- [x] Fix `fe-react/security.md`
- [x] Fix `fe-react/state-management.md`
- [x] Validate: `go run ./apps/rhino-cli/main.go docs validate-mermaid 2>&1 | grep "fe-react/"` → no output
- [x] Commit: `fix(docs): fix mermaid violations in fe-react/ docs (batch 6/10)`

<!-- 2026-04-25 | Status: Done | All 8 files fixed; zero violations confirmed -->

## Batch 7 — Next.js (3 files — verified from Phase 0 re-audit)

Files: `docs/explanation/software-engineering/platform-web/tools/fe-nextjs/`

- [x] Fix `fe-nextjs/README.md`
- [x] Fix `fe-nextjs/app-router.md`
- [x] Fix `fe-nextjs/rendering.md`
- [x] Validate: `go run ./apps/rhino-cli/main.go docs validate-mermaid 2>&1 | grep "fe-nextjs/"` → no output
- [x] Commit: `fix(docs): fix mermaid violations in fe-nextjs/ docs (batch 7/10)`

<!-- 2026-04-25 | Status: Done | All 3 files fixed; zero violations confirmed -->

## Batch 8 — Elixir Language (7 files — verified from Phase 0 re-audit)

Files: `docs/explanation/software-engineering/programming-languages/elixir/`

- [x] Fix `elixir/README.md`
- [x] Fix `elixir/ddd-standards.md`
- [x] Fix `elixir/otp-application.md`
- [x] Fix `elixir/otp-genserver.md`
- [x] Fix `elixir/otp-supervisor.md`
- [x] Fix `elixir/protocols-behaviours-standards.md`
- [x] Fix `elixir/testing-standards.md`
- [x] Validate: `go run ./apps/rhino-cli/main.go docs validate-mermaid 2>&1 | grep "programming-languages/elixir/"` → no output
- [x] Commit: `fix(docs): fix mermaid violations in elixir/ docs (batch 8/10)`

<!-- 2026-04-25 | Status: Done | All 7 files fixed; zero violations confirmed -->

## Batch 9 — C4 Architecture (5 files — verified from Phase 0 re-audit)

Files: `docs/explanation/software-engineering/architecture/c4-architecture-model/`

- [x] Fix `c4-architecture-model/README.md`
- [x] Fix `c4-architecture-model/bounded-context-visualization.md`
- [x] Fix `c4-architecture-model/diagram-standards.md`
- [x] Fix `c4-architecture-model/notation-standards.md`
- [x] Fix `c4-architecture-model/nx-workspace-visualization.md`
- [x] Validate: `go run ./apps/rhino-cli/main.go docs validate-mermaid 2>&1 | grep "c4-architecture-model/"` → no output
- [x] Commit: `fix(docs): fix mermaid violations in c4-architecture-model/ docs (batch 9/10)`

<!-- 2026-04-25 | Status: Done | All 5 files fixed; zero violations confirmed -->

## Batch 10 — Remaining errors (15 files — verified from Phase 0 re-audit)

- [x] Fix `docs/explanation/software-engineering/programming-languages/c-sharp/README.md`
- [x] Fix `docs/explanation/software-engineering/programming-languages/clojure/README.md`
- [x] Fix `docs/explanation/software-engineering/programming-languages/f-sharp/README.md`
- [x] Fix `docs/explanation/software-engineering/programming-languages/java/README.md`
- [x] Fix `docs/explanation/software-engineering/programming-languages/kotlin/README.md`
- [x] Fix `docs/explanation/software-engineering/programming-languages/rust/README.md`
- [x] Fix `docs/explanation/software-engineering/platform-web/tools/jvm-spring/README.md`
- [x] Fix `docs/explanation/software-engineering/platform-web/tools/jvm-spring/web-mvc.md`
- [x] Fix `docs/explanation/software-engineering/development/README.md`
- [x] Fix `docs/reference/project-dependency-graph.md`
- [x] Fix `docs/reference/system-architecture/README.md`
- [x] Fix `docs/reference/system-architecture/applications.md`
- [x] Fix `docs/reference/system-architecture/ci-cd.md`
- [x] Fix `docs/reference/system-architecture/components.md`
- [x] Fix `docs/reference/system-architecture/deployment.md`
- [x] Validate: `go run ./apps/rhino-cli/main.go docs validate-mermaid 2>&1 | grep -E "(c-sharp|clojure|f-sharp|java/|kotlin|rust/|jvm-spring/|project-dependency-graph|system-architecture|software-engineering/development)"` → no output
- [x] Commit: `fix(docs): fix mermaid violations in remaining docs (batch 10/10)`

<!-- 2026-04-25 | Status: Done | All 15 files fixed; zero violations confirmed -->

## Final Validation

> **Important**: Fix ALL failures found during quality gates, not just those caused by
> your changes. This follows the root cause orientation principle — proactively fix
> preexisting errors encountered during work. Do not defer or mention-and-skip existing
> issues.

- [ ] Run full validator: `go run ./apps/rhino-cli/main.go docs validate-mermaid`
- [ ] Confirm zero lines starting with `✗` (warnings tolerated)
- [ ] Confirm `[width_exceeded]` count = 0
- [ ] Confirm `[label_too_long]` count = 0
- [ ] Confirm prd.md Scenarios 1–4 (direction-aware discrimination) are covered by new
      test cases in `validator_test.go`
- [ ] Run affected quality gates: `npx nx affected -t typecheck lint test:quick spec-coverage`
- [ ] Fix ALL failures found — including preexisting issues not caused by your changes

### Commit Guidelines

- Follow Conventional Commits format: `<type>(<scope>): <description>`
- Keep diagram fixes in `fix(docs):` commits; stage and commit separately any
  unrelated preexisting fixes discovered during the batch work
- Do not bundle unrelated concerns in a single commit
- One commit per batch is the default; split only if unrelated changes exist

### Post-Push Verification

- [ ] Push changes to `main`
- [ ] Monitor GitHub Actions on the ose-primer repository after push — ose-primer has
      active CI workflows (pr-quality-gate.yml, test-demo-be-\*, test-demo-fe-\*, and
      others under `.github/workflows/`)
- [ ] If push is to a PR branch: verify `pr-quality-gate.yml` passes
- [ ] If push is directly to `main`: verify no push-trigger workflows fail
- [ ] Confirm no regressions introduced by diagram changes before closing the plan

## Phase 2 — Governance Propagation (repo-rules-maker)

> **Agent**: Use `repo-rules-maker` for all edits in this phase.

### 2a — Update `governance/conventions/formatting/diagrams.md`

> See tech-docs.md Phase 2 section for the full content specification for each item above.

- [ ] 2a-1: Invoke `repo-rules-maker` to add `### Flowchart Width Constraints (Automated
Enforcement)` subsection inside the "Mermaid Best Practices" area (after "Mermaid
      Best Practices" heading, before any sub-sections on size):
      — MaxWidth=4 (horizontal only, direction-aware)
      — Span vs. depth defined with LR/TD direction mapping table
      — Label length: 30 raw chars (validator rule) vs. ~20 chars (Hugo/Hextra clip)
      — `go run ./apps/rhino-cli/main.go docs validate-mermaid` command
- [ ] 2a-2: Invoke `repo-rules-maker` to add `## Width Violation Fix Strategy Guide` H2
      section after "Flowchart Width Constraints", containing:
      — Selection decision tree (label_too_long → Strategy 4; width_exceeded → try
      direction flip first, then structural fix)
      — Strategy 0: Direction flip — condition `min(span,depth) ≤ 4`, one-word fix
      — Strategy 1: Intermediate grouping node (real edges, NOT subgraph)
      — Strategy 2: Diagram splitting (reference existing "Diagram Size and Splitting")
      — Strategy 3: Sequential chaining
      — Strategy 4a/4b: Label shortening (entity replacement + abbreviation)
      — Subgraph warning: parser skips subgraph/end lines; fixes must be topological
- [ ] 2a-3: Invoke `repo-rules-maker` to update the "Diagram Orientation" section:
      change the absolute "MUST use graph TD" rule to "TD by default; LR is preferred
      when it reduces horizontal width below MaxWidth=4 (Strategy 0)"
- [ ] 2a-4: Invoke `repo-rules-maker` to remove duplicate Error 7 (identical content to
      Error 5)
- [ ] 2a-5: Invoke `repo-rules-maker` to clarify label length note in Error 9 /
      Verification Checklist: "30 raw chars = validator enforcement limit; ~20 chars =
      Hugo/Hextra rendering clip"

### 2b — Validate and commit

- [ ] Validate updated diagrams.md passes the mermaid validator:
      `go run ./apps/rhino-cli/main.go docs validate-mermaid governance/conventions/formatting/`
      → no `✗` output
- [ ] Run markdown lint: `npm run lint:md` → 0 errors
- [ ] Commit: `docs(governance): add mermaid width constraints and fix strategies to diagrams convention`

### 2c — Run repo-rules quality gate in strict mode

- [ ] Invoke `repo-rules-quality-gate` workflow with `mode=strict`
      (see `governance/workflows/repo/repo-rules-quality-gate.md`)
- [ ] Fix ALL CRITICAL, HIGH, and MEDIUM findings from the quality gate
- [ ] If fixes needed: commit with `docs(governance): fix repo-rules quality gate findings`
- [ ] Confirm quality gate passes with zero CRITICAL/HIGH/MEDIUM findings on two
      consecutive passes

## Plan Archival

- [ ] Verify ALL delivery checklist items are ticked
- [ ] Verify ALL quality gates pass (local)
- [ ] Move `plans/in-progress/2026-04-25__fix-mermaid-validation-and-violations/` → `plans/done/` via `git mv`
- [ ] Update `plans/in-progress/README.md` — remove entry
- [ ] Update `plans/done/README.md` — add entry with completion date
- [ ] Commit: `chore(plans): move fix-mermaid-validation-and-violations to done`

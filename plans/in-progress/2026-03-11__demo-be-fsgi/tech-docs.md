...
`demo-be-jasb`'s MockMvc approach). The database layer uses EF Core's SQLite in-memory
...
refresh tokens (long-lived) follow the same pattern as demo-be-jasb:
...
```json
{
  ...
  "sourceRoot": "apps/demo-be-fsgi/src",
    ...
    "test:quick": {
      ...
      "options": {
        "commands": [
          ...
          "(cd ../../apps/rhino-cli && CGO_ENABLED=0 go run main.go test-coverage validate apps/demo-be-fsgi/coverage/**/coverage.info 90)",
...
### Docker Compose (`infra/dev/demo-be-fsgi/docker-compose.yml`)
...
services:
  demo-be-db:
    ...
    environment:
      POSTGRES_DB: demo_be_fsgi
      POSTGRES_USER: demo_be_fsgi
      POSTGRES_PASSWORD: demo_be_fsgi
    ports:
      - "5432:5432"
    volumes:
      - demo-be-fsgi-db-data:/var/lib/postgresql/data
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U demo_be_fsgi"]
      interval: 5s
      timeout: 3s
      retries: 5
    networks:
      - demo-be-fsgi-network

  demo-be-fsgi:
    build:
      context: .
      dockerfile: Dockerfile.be.dev
    container_name: demo-be-fsgi
    ports:
      - "8201:8201"
    environment:
      - ASPNETCORE_URLS=http://+:8201
      - DATABASE_URL=Host=demo-be-db;Database=demo_be_fsgi;Username=demo_be_fsgi;Password=demo_be_fsgi
      - APP_JWT_SECRET=dev-jwt-secret-at-least-32-chars-long
    volumes:
      - ../../../apps/demo-be-fsgi:/workspace:rw
...
### New Workflow: `e2e-demo-be-fsgi.yml`
...
- name: Upload coverage — demo-be-fsgi
  uses: codecov/codecov-action@v5
  with:
    token: ${{ secrets.CODECOV_TOKEN }}
    files: apps/demo-be-fsgi/coverage/**/coverage.info
    flags: demo-be-fsgi

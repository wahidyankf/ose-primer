...
- **Integration tests** (`test:integration`): ASP.NET Core TestServer + in-memory database
  ...
  Matches the monorepo convention established by `demo-be-jasb` (MockMvc + InMemoryDataStore)
  and `demo-be-exph` (InMemoryStore contexts).
...
- Port 8201 (mutually exclusive with demo-be-jasb and demo-be-exph)

...
- **Per-Repository Delivery Mode Restrictions (HARD RULE)**: `main` is branch-protected against direct pushes (including for admins) in `ose-public`, `ose-primer`, and `beaver-nest` — `worktree-to-origin-main` and `main-to-origin-main` have **no executable path** in those three repositories; `worktree-to-pr` is the only applicable mode there for routine development. In `ose-private`, both direct-push modes remain available **only** for infrastructure-as-code plans (Terraform, Ansible, and equivalent state-changing infra work needing the primary checkout's real secrets and local state). Flag as **HIGH** a plan's `## Delivery Mode` field (or an invocation argument) declaring `worktree-to-origin-main` or `main-to-origin-main` for a target repository in `ose-public`, `ose-primer`, or `beaver-nest`; also flag as **HIGH** the same declaration for `ose-private` when the plan is not infrastructure-as-code. See [Plans Organization Convention §Per-Repository Delivery Mode Restrictions](../../repo-governance/conventions/structure/plans.md#per-repository-delivery-mode-restrictions-hard-rule).
...
9. **Delivery mode is repo-executable (Per-Repository Delivery Mode Restrictions)** — for each
   ...
   branch-protected against direct pushes in `ose-public`, `ose-primer`, and `beaver-nest`, so
   `worktree-to-origin-main` and `main-to-origin-main` have no executable path there at all —
   `worktree-to-pr` is the only applicable mode. In `ose-private`, both direct-push modes remain
...
- `## Delivery Mode` (declared or defaulted) names a mode with no executable path for a target
  ...
  `beaver-nest`; either direct-push mode in `ose-private` on a non-infrastructure-as-code plan:
  **HIGH**

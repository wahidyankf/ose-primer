---
name: apps-ayokoding-fs-by-example-fixer
description: Applies validated fixes from apps-ayokoding-fs-by-example-checker audit reports. Re-validates By Example findings before applying changes. Use after reviewing checker output.
...
# By Example Tutorial Fixer for ayokoding-fs
...
**Domain-Specific Implementation**: This agent re-validates By Example tutorial findings focusing on annotation density (1-2.25 ratio per example), five-part structure, example count (75-85), and ayokoding-fs compliance.
...
- `apps-ayokoding-fs-by-example-maker` - Creates content
- `apps-ayokoding-fs-by-example-checker` - Validates content (generates audits)

...
available, but only
for an infrastructure-as-code plan (Terraform, Ansible, and equivalent state-changing infra work
needing the primary checkout's real secrets and local state), and only as an explicit selection, never
an inferred one; `main-to-origin-main` specifically carries a further content restriction on top of
that — valid only for an `.md`-only change set or with explicit user go-ahead (Standard 2). This
applies to all contexts: general work, plan creation, plan checking, plan fixing, and plan execution.
...
secrets and local state). Every other plan, in all four repositories, uses `worktree-to-pr`. See

...
- [Trunk Based Development](./trunk-based-development.md) - Git workflow for this repository; the default is direct push to `main` regardless of execution context. See the [Worktree Mode (Direct Push to main; Draft PR Opt-In)](./trunk-based-development.md#worktree-mode-direct-push-to-main-draft-pr-opt-in) section for the decision table on when a draft PR is used instead.
- [Git Push Default Convention](./git-push-default.md) — Direct push to `origin main` is the default; plan delivery checklist steps for worktree creation, push, and removal MUST be tagged `[AI]` (see [Plans Organization Convention §Executor Tagging](../../conventions/structure/plans.md#executor-tagging--ai-vs-human-hard-rule)).

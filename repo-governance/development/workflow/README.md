...
- [Bare-Repo Base-Worktree Landing Method](./bare-repo-landing-method.md) - The procedure for landing changes into a repository with no primary checkout (today `ose-primer` and `ose-private`): verify topology first (`git worktree list`, never `git rev-parse --is-bare-repository`), the seven-step base-worktree landing sequence, the topology-keyed terminal reconcile (`git fetch origin main:main` bare, `git merge --ff-only origin/main` where a work tree exists) that closes the silent local-`main`-lag defect, the one-landing-path-per-unit-of-work rule, and advisory (non-enforced) guidance for parking long-lived WIP on a `refs/heads/wip/*` branch
- [Integration Diff Review Convention](./integration-diff-review.md) - After any rebase, pull, merge, cherry-pick, or fast-forward lands foreign commits on the current branch, read the full incoming diff and reassess impact on in-flight work before continuing — a clean, conflict-free integration is not proof the incoming changes are safe to ignore

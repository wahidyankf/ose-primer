...
- [Git Fixture Isolation Convention](./git-fixture-isolation.md) - Defense-in-depth mandate (six mandatory layers: capped discovery, explicit `GIT_DIR`/`GIT_WORK_TREE`, blanked identity/config, pre-write escape guard, exit-status checking, throwaway-clone-only diagnosis) for any test fixture that shells out to `git` to build throwaway repositories, so a fixture can never mutate the real repository

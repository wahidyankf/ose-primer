...
package.json scripts is implemented in `apps/rhino-cli/`.
...
| shellcheck, hadolint, and actionlint all run inside the `shell-docker-actions` CI gate group (not separate per-tool job keys)                                            | `pr-quality-gate.yml` job keys           |

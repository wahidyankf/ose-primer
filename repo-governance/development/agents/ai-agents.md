...
**Translation table** (mirrors `ClaudeToOpenCodeColor` in `apps/rhino-cli-go/internal/agents/types.go`):
...
**Single source of truth**: `apps/rhino-cli-go/internal/agents/types.go` — `ClaudeToOpenCodeColor` map. Any change to the mapping MUST update both the map and this table in the same commit.

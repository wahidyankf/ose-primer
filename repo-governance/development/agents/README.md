...
- [Subagent Orchestration Convention](./subagent-orchestration.md) - The N+1 concurrency model (`1 main thread + N background agents`, default N=3, adjustable per-plan and never self-promoted) and 3-minute mtime-based stuck-detection polling when a main agent spawns background subagents via the Agent tool; controls token burn rate, prevents per-minute model-vendor rate-limit collisions, and detects stalled background agents

...
- [Git Push Default Convention](./workflow/git-push-default.md) - Default push behavior: the repo-wide default integration target is a PR branch opened against `main` (`worktree-to-pr`); the direct-push modes remain fully available where a plan declares them. Governs plan-maker, plan-checker, plan-fixer, and the plan-execution workflow — the push itself is always `[AI]` in every mode, and no "review the diff and approve push" gate belongs in a delivery checklist, since pushing to a PR branch is not a merge
...
- [No Destructive Git Operations Convention](./workflow/no-destructive-git-operations.md) - Forbids local destructive and irreversible git operations (hard reset, recursive clean, force branch delete, history rewrite of shared refs) that can discard a concurrent actor's uncommitted work on a shared machine, and prescribes the non-destructive equivalent for each. The local-side companion to the Git Push Safety Convention, which owns the remote side
- [Bare-Repo Base-Worktree Landing Method](./workflow/bare-repo-landing-method.md) - The procedure for landing changes into a repository with no primary checkout (today `ose-primer` and `ose-private`): verify topology first (`git worktree list`, never `git rev-parse --is-bare-repository`), the seven-step base-worktree landing sequence, the topology-keyed terminal reconcile that closes the silent local-`main`-lag defect, the one-landing-path-per-unit-of-work rule, and advisory guidance for parking long-lived WIP on a `refs/heads/wip/*` branch
- [Worktree and Artifact Cleanup Convention](./workflow/worktree-and-artifact-cleanup.md) - Mandatory plan-end gate requiring a plan to remove the worktrees, branches, and build artifacts it created. Cleanup is self-scoped (only what the plan created), verified idle before each removal, and never touches shared caches other sessions depend on. The teardown half of the worktree lifecycle whose provisioning half is Worktree Toolchain Initialization
...
- [PR Merge Protocol Convention](./workflow/pr-merge-protocol.md) - Practice gating PR merges on five hardened preconditions rather than a per-instance prompt; `[AI]` merges by default, `[HUMAN]` is an explicit per-plan opt-in, and all quality gates must pass before merge
...
- [CI Post-Push Verification Convention](./workflow/ci-post-push-verification.md) - After pushing app or lib code — to a PR branch under the default `worktree-to-pr`, or to `origin main` under the direct-push modes — manually trigger all related GitHub CI workflows and verify they pass before considering the work complete
...
- [PR Reviewer-Discipline Convention](./quality/pr-review-disciplines.md) - Defines the eight PR-review specialist disciplines and their owned/routed-to scope, the boundary tie-breaker rule (documented rule → governance, new tradeoff → architecture, domain-intent → correctness), six grey-zone rulings, and the Cloudflare-derived cost- and noise-control mechanics (risk-tier fan-out, shared-context extraction, per-specialist `SUPPRESS` blocks, instruction-decay coverage) governing the specialist-plus-coordinator PR review pipeline
...
- [Subagent Orchestration Convention](./agents/subagent-orchestration.md) - The N+1 concurrency model applied to background Agent-tool spawns (N background agents plus the main thread as the +1; N defaults to 3, adjustable per-plan and never self-promoted), 3-minute stuck-detection polling, result handling, and orchestration patterns

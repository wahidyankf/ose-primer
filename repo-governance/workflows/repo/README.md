...
- [Repository Dependency Bump Planning](./repo-dependency-bump-planning.md) - `planning`-type workflow that surveys every dependency manifest across the monorepo (npm, Cargo, .NET, Go, Docker, GitHub Actions), classifies each candidate bump per the [Dependency Bump Stability & Safety Policy](../../development/workflow/dependency-bump-policy.md) (three-path tree, CISA KEV / EPSS, Rule 5a/5b), and hands the results to `plan-planning` to author a validated **backlog** plan. Never edits a manifest or lockfile — the deliverable is the plan.
- [Harness Compatibility Quality Gate](./repo-harness-compatibility-quality-gate.md) - The single harness-compat gate. **Phase 0** runs 5 deterministic cross-vendor parity invariants (governance vendor-neutrality, AGENTS+CLAUDE, binding sync no-op over `.opencode/` + `.amazonq/`, agent count parity, color + tier maps); **Phase 1** detects external drift between each supported harness's current upstream conventions and the platform-bindings catalog plus committed binding files (agent-backed, web-research-backed). Complements the deterministic `rhino-cli harness bindings validate` / `validate:cross-vendor-parity` pre-push guard. Iterative checker→fixer until two consecutive zero-finding runs.

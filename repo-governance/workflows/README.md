...
| [Plan Planning](./plan/plan-planning.md)                                                                                  | Transform a user prompt into a production-ready plan: repo exploration → grill → web research → grill → plan-maker → plan-quality-gate → push                                                                                                                                                                                                                                                                                                                                                                     | plan-maker, web-researcher, plan-checker, plan-fixer                                                  | High       |
...
| [Plan Idea Promotion Planning](./plan/plan-idea-promotion-planning.md)                                                    | Promote one ripe `plans/ideas/` two-pager into a full five-document backlog plan: completeness/ripeness gate → the deferred deep prior-art `web-researcher` study → promotion checkpoint → `plan-planning` (target-stage=backlog) → retire the two-pager (delete + de-index). A not-yet-ripe brief gets a readiness report and no plan. Deliverable is the plan, never the implementation.                                                                                                                        | web-researcher, plan-maker, plan-checker, plan-fixer                                                  | Medium     |
...
| [Plan Ideas Grooming](./plan/plan-ideas-grooming.md)                                                                      | Sweep and reorganize `plans/ideas/` across repos — dedup merge/split, cross-repo residency correction, Eisenhower-quadrant classification, two-pager reshape, and rename-with-link-rewrite, on a recurring file-count/elapsed-time trigger                                                                                                                                                                                                                                                                        | none — direct orchestration, mechanical file operations                                               | Medium     |
| [Multi-Plans Execution](./plan/multi-plans-execution.md)                                                                  | Execute several plans together — an explicit list or a set-selector (`all-in-progress` / `all-backlog` / `all`, optionally minus an `except` list) resolved to a frozen set: build a dependency DAG (explicit `Depends-on` wins, resource-overlap inference fills gaps), materialize one very-granular union Task list, and run a bounded ready-queue scheduler (default 3 parallel nodes, overridable) driving each plan through its full plan-execution lifecycle; failure quarantines a plan without cascading | plan-execution-checker, plan-checker, plan-fixer, pr-review-synthesis-maker, pr-review-fixer          | High       |
| [Plan Handover Execution](./plan/plan-handover-execution.md)                                                              | Write a structured handover document (`local-tmp/handovers/<date>__<plan-identifier>-implementation.md`) capturing an in-progress plan's per-repo state, concrete next step, and non-obvious gotchas learned this session, for a different agent or session to resume from. Local, gitignored, single-machine artifact; read by plan-take-over-execution as a lead, never a substitute for its own verification                                                                                                   | none — direct orchestration                                                                           | Low        |
| [Plan Take-Over Execution](./plan/plan-take-over-execution.md)                                                            | Given a plan path, discover its true execution state across the current repo and its siblings (`ose-public`, `ose-private` at minimum) — worktrees, local/remote branches, PRs — reconcile findings, take over any live in-flight work, clean up confirmed-stale leftover worktrees/branches/build artifacts, then hand off to plan-execution                                                                                                                                                                     | none — direct orchestration, hands off to plan-execution-checker                                      | Medium     |
| [Plan Multi-Repo Parity Planning](./plan/plan-multi-repo-parity-planning.md)                                              | Survey multiple sibling repos, build a cross-repo deviation matrix, grill every gap to a recorded decision (first grill, hard gate), verify external claims via web research, re-grill on the findings, then author one aligned-but-deliberately-divergent plan per repo and gate each to double-zero                                                                                                                                                                                                             | plan-maker, web-researcher, plan-checker, plan-fixer                                                  | High       |
| [Plan Multi-Repo Parity Planning and Execution](./plan/plan-multi-repo-parity-planning-and-execution.md)                  | End-to-end composite: full parity planning (survey, matrix, two grills, research, author, gate, deliver), then a third pre-execution grill, then plan-execution per repo to zero findings — flattened granular Task list synced 1:1 with each delivery.md, archival, and prompted worktree cleanup                                                                                                                                                                                                                | plan-maker, web-researcher, plan-checker, plan-fixer, plan-execution-checker, repo-setup-manager      | High       |
...
| [Web UX Test-Fixing Planning](./web/web-ux-test-fixing-planning.md)                                                       | Run the three live-site testers — `web-exploratory-tester` (spec-aware correctness), `web-usability-tester` (spec-blind usability), and `web-design-tester` (design-aware fidelity) — against the same live URL(s) and goal, then synthesize all three result sets into one fix-ready plan — findings stay source-attributed (EWT-### vs UWT-### vs DWT-###), plus `tech-docs.md` and a TDD-shaped `delivery.md`. Deliverable is the plan, not the fixes.                                                         | web-exploratory-tester, web-usability-tester, web-design-tester, plan-maker, plan-checker, plan-fixer | High       |
...
| `planning`     | Single forward planning procedure whose terminal deliverable is a plan document                                                                                                                                                                           | `repo-dependency-bump-planning`                              |
| `grooming`     | Recurring sweep/reorganization workflow over already-existing documentation or artifact state; does not converge to zero findings, does not produce a new plan, and is not one-time provisioning — re-sweeps existing docs on a stated cadence or trigger | `plan-ideas-grooming`                                        |
...
- **plan**: Project planning documents — including two-pager→backlog idea promotion; `plan-ideas-grooming` grooms the idea corpus itself (`plans/ideas/`), sweeping and reorganizing it rather than authoring or executing a plan
...
Workflows for live-website testing and fix-planning ([web/README.md](./web/README.md)):
...
- **repo-harness-compatibility**: Validate internal cross-vendor parity invariants (Phase 0) between primary (`.claude/`) and secondary (`.opencode/`, `.amazonq/`) bindings, and detect external harness-convention drift (Phase 1) against the platform-bindings catalog/binding files
- **repo-dependency-bump-planning**: Survey all dependency manifests across the monorepo, classify each candidate bump per the Dependency Bump Stability & Safety Policy, and author a validated backlog plan (never edits a manifest)

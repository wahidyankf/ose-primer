...
| **Harness compatibility gate**   | `repo-harness-compatibility-quality-gate` | **Phase 0 — internal cross-vendor parity** (5 deterministic invariants: governance/root-surface vendor-neutrality, binding sync no-op over `.opencode/` + `.amazonq/`, agent count parity, color/tier-map coverage) AND **Phase 1 — external convention drift** (whether each harness's current upstream documentation matches the platform-bindings catalog and committed binding files) | Phase 0 offline/deterministic; Phase 1 web-research-backed, agent-driven | Monthly cadence; after a harness ships a major release; before adding a new harness |

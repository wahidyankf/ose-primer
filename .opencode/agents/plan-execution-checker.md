...
2. **Post-Push CI Passed**
   - Check if GitHub Actions workflows passed for the latest commits on the plan's delivery target — the PR branch under `*-to-pr` modes, `main` under the direct-push modes (resolve the mode first; do not assume `main`). `.github/workflows/main-ci.yml` no longer exists (removed — its checks were folded into the gate registry driving `pr-quality-gate.yml`)
...
6. **Per-Repository Delivery Mode Restrictions were actually honored during execution** — for each
   ...
   infrastructure-as-code. A direct push that landed in a protected repository (it would only
   succeed if branch protection was itself misconfigured or bypassed) is **CRITICAL** — it means a
   HARD RULE was violated at the platform level, not just under-documented. A non-IaC `ose-private`
   plan that used a direct-push mode: **HIGH**. See
